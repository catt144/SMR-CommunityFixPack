...
WHY THIS EXISTS
    `SMRFixPack_Disabled`, `SMRFixPack_Optional` and `SMRFixPack` are not private
...
WHAT IT DOES
    ...
    containment reproduced:
      ...
      * a file that throws never reaches its `Register`, so its id is ABSENT from
        `SMRFixPack.fixes` and `order` — not `inactive`. The harness reports
...
#   * DustDevilSpawnGate  — a global replacement, and the pack's own restore donor
#   * MeteorFrequency     — a GameVar owner with an OnMsg heal path
#   * ExtenderFlapChurn   — the one module that installs OUTSIDE apply's pcall
MODULES = ["00_Core.lua",
           "Fix_DustDevilSpawnGate.lua",
           "Fix_MeteorFrequency.lua",
...
CASES = [
    ...
    ("CONTROL — the documented form",
     ...
     "README.md:76-77, verbatim. Must veto exactly one module."),

    ("CONTROL — nothing set at all",
     '',
     "the shipping case: no foreign mod, no console. Must veto nothing."),

    ("SMRFixPack_Disabled = true",
     'SMRFixPack_Disabled = true',
     'the natural misreading of "set SMRFixPack_Disabled to disable a fix"'),

    ('SMRFixPack_Disabled = "DustDevilSpawnGate"',
     'SMRFixPack_Disabled = "DustDevilSpawnGate"',
    ...
    ("SMRFixPack = {} (a fork or an older copy)",
     'SMRFixPack = {}',
     "a fork, a second copy of the pack, or a shim that reserves the table"),

    ("SMRFixPack = {fixes={}, order={}} (partial)",
     'SMRFixPack = { fixes = {}, order = {} }',
     "a shim that knows the two documented sub-tables and not the two later ones"),

    ("SMRFixPack_Disabled with a throwing __index",
     'SMRFixPack_Disabled = setmetatable({}, {__index = function() error("hostile") end})',
...
def run_case(seed):
    ...
    reg = {}
    have = lua.eval('type(SMRFixPack) == "table" and type(SMRFixPack.fixes) == "table"')
    if have:
        ids = lua.eval("SMRFixPack.order")
        for i in range(1, int(lua.eval("#SMRFixPack.order")) + 1):
            fid = ids[i]
    ...
    for label, seed, why in CASES:
        ...
        for line in log:
            if "disabled by user/mod setting" in line or line.startswith("SIMERROR"):
                print("      log   %s" % line.replace("[CommunityFixPack] ", ""))
