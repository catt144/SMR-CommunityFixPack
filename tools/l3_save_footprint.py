...
# `modtable` our own tables (SMRFixPack.*). Mod env, never persisted.
...
NAMED_STATE = re.compile(r'\bSMRFixPack_(\w+)')
...
REGISTER = re.compile(r'SMRFixPack\.Register\s*\(\s*(?:["\'](\w+)["\']|(\w+))')
...
def classify(receiver, aliases, locals_, path_name):
    ...
    if resolved.startswith("SMRFixPack"):
    ...
    for rel in files:
        ...
        for n, raw, code in code_lines(path):
            ...
            for m in NAMED_STATE.finditer(code):
                named["SMRFixPack_" + m.group(1)].append((name, n))
