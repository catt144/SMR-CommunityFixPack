...
GENERATED_BANNER = ("<!-- GENERATED — never hand-edit; regenerate with: "
...
def render_index(model):
    ...
    out = [
        GENERATED_BANNER,
        "<!-- split_bugs.py --write is the one-time MIGRATION from the retired "
        ...
        "",
        "%d F + %d D + %d C. `seq` is the entry's position in the old BUGS.md "
        "file order;" % (kinds["F"], kinds["D"], kinds["C"]),
        "the row order below is the old index table's own. Generated from the "
        "front matter of",
