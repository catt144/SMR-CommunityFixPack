...
Censuses
  ...
  3  veto       — every site that runs pack work, tagged with which of the three
                  ...
                  — everything else is a site `SMRFixPack_Disabled` does not stop
...
REGISTER = re.compile(r'SMRFixPack\.Register\(\s*(?:"([^"]+)"|([A-Za-z_][\w]*))')
...
def register_span(text):
    """(start, end) character offsets of the SMRFixPack.Register(...) call.
...
SELF_VETO = re.compile(r'SMRFixPack_Disabled')
...
def scan_module(path):
    ...
    for kind, rx in SITE_PATTERNS:
        for m in rx.finditer(text):
            ...
            if kind == "method_decl":
                # only class/table method installs matter; SMRFixPack.* helpers
                # are our own namespace, not a patch on the game
                if m.group(1) == "SMRFixPack":
                    continue
...
def main():
    ...
    print()
    print("=" * 78)
    print("CENSUS 3 — the veto route: which sites SMRFixPack_Disabled does NOT stop")
    print("=" * 78)
    kinds = {}
    for m in mods:
        for s in m["sites"]:
