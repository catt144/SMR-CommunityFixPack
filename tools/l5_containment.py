...
FILESCOPE_KINDS = [
    ...
    ("register", re.compile(r"^SMRFixPack\.Register\s*\(")),
...
#   * SMRFixPack.* — 00_Core.lua is first in metadata `code`, so it is loaded
...
RE_ENTRY = {
    "register": re.compile(r"SMRFixPack\.Register\s*\(\s*([A-Za-z_\"][\w\"]*)"),
    "datapatch": re.compile(r"SMRFixPack\.DataPatch\s*\("),
    "ondataready": re.compile(r"SMRFixPack\.OnDataReady\s*\("),
    "whenactive": re.compile(r"SMRFixPack\.WhenActive\s*\("),
    "onmsg-raw": re.compile(r"^\s*(?:function\s+)?OnMsg\.(\w+)"),
    "setglobal": re.compile(r"SMRFixPack\.SetGlobal\s*\("),
    ...
    for name in code_files():
        ...
        for n, raw in enumerate(lines, 1):
            ...
            if m and name != "00_Core.lua":
                ...
                # delegation in this tree is `pcall(SMRFixPack.MigrateRainsState)`,
