...
--
-- Two Mod Options dropdowns (Options → Mod Options → Community Fix Pack):
...
--     dial positions).
--   * Base position, veto (SMRFixPack_Disabled) or error status = modifiers
...
local function DesiredDials()
	...
	if SMRFixPack_Disabled[ID] or not SMRFixPack.IsActive(ID) then
		return 0, 0
	end
...
if not install_error then
	...
	function OnMsg.ApplyModOptions(mod_id)
		if mod_id == "SMR_CommunityFixPack" then
			ReapplyDials()
		end
	end
