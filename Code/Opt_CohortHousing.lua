...
-- SMRFixPack.IsActive per call and passes through while off). Other mods /
-- power users can pre-seed SMRFixPack_Optional = { CohortHousing = true }.
--
...
local function module_active()
	return SMRFixPack.IsActive("CohortHousing")
end
...
SMRFixPack.Register("CohortHousing", {
	title = "OPTIONAL: Seniors/Children auto-move to free Retirement Home/Nursery slots",
	optional = true,
	apply = function()
		if not SMRFixPack.OptionEnabled("CohortHousing") then
