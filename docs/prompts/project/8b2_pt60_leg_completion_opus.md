...
| ~~5~~ | ~~`f90`~~ | ✅ **F90 DONE 2026-08-02 — B1-B5 ALL PASS.** Pack ON, same fixture, same storm: **underground 0 on all 26 samples** against **15** pack-OFF; surface still broke 0→5; **zero `[LUA ERROR]`** incl. none from the empty filtered list; post-storm census `frag 1: connectors 1668` unchanged (persisted field restored). ⭐ ~70% of the storm watched from underground — camera-independent both ways. **Nothing further owed** | — |
| 6 | `TEST 2H` | ⚠️ **MOSTLY DONE 2026-08-02 — only P6/P7 (unpaused play) may still be owed.** ✅ **P4/P5**: `76 PASS, 0 FAIL, 9 SKIP, 0 ERROR` (85 = probe count), **all seven new probes PASS**, no pre-existing probe regressed. ✅ **F92 heal**: `re-based 10 dome blessing(s)`, **10 of 10** registered, line appears **once** across a reload and the count stayed 10 — idempotent. **Remaining:** ~15 min unpaused play → **P6** zero `[LUA ERROR]` naming any new or converted module, **P7** no visible behaviour change from the eight conversions | see left |
