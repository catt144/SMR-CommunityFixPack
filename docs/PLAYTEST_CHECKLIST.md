...
| **C20 pause-scan observation** ⭐ **ADDED 2026-08-02 (prompt 6b) — no console, no mystery, no TestKit; any save with an Orbital Probe and an unexplored sector** | **The proxy that makes this free:** `AddHUDNotification("SectorScanned", …)` is the line immediately before `Msg("SectorScanned", …)` in the same thread with no yield between (`Exploration.lua:103-104`), so **the on-screen "Sector scanned" toast appears if and only if the Philosopher's Stone counter ticks.** **Steps:** (1) **Pause the game.** (2) Open overview and deploy a probe on an unexplored sector. (3) **Still paused**, watch for the "Sector scanned" toast — expected **absent** (the emitter is a game-time thread that opens with `Sleep(10)`, `Exploration.lua:88-89`). (4) Unpause and watch again. **The verdict is in step 4, and there are three of them:** toast appears immediately on unpause → **DEFERRED, harmless, close C20**; toast never appears → **LOST, a real silent defect, promote**; the probe refuses to deploy while paused → **unreachable, close C20**. (5) **Only if step 4 said "deferred", the second half:** repeat 1-2, then **save and reload while still paused**, unpause, and watch — a toast that never arrives means the count is lost across a save, which is the same defect by a slower route. ⚠️ **Note which sector you scanned**; a probe scans a whole pattern at once, so count the toasts against the number of sectors that changed state — fewer toasts than sectors is itself a finding (the counter's single `WaitMsg` loop may not re-arm in time). Decides C20 (BUGS C20 entry) |
| **F82 timing observation** ⭐ **REWRITTEN 2026-08-02 — the source trace is DONE, so this is now a one-shot confirmation with a named number, not an investigation** | **Stopwatch REAL seconds, not sols, and write down the game speed.** The trace (F82 entry, prompt 6c) says the notification has no removal path at all and expires on a **real-time** timer of **120 s**, so: **(a)** split a power or life-support grid, start a real stopwatch, and **do not rejoin it** — the notification should clear on its own at **≈120 real seconds**, which is the half the original report never saw; **(b)** repeat, rejoining the grid within the first few seconds — the notification should **still** clear at ≈120 real seconds from the *split*, unmoved by the rejoin. **PASS = both ≈120 s ±15 s.** ⭐ **The discriminator is game speed**: run (a) once at normal and once at ultra. Real seconds should be the **same** both times while the elapsed **sols** differ by ~20× (≈1/6 sol at normal, ≈3.3 sols at ultra). Any state-cleared notification would do the opposite. **FAIL/surprise = the rejoin visibly shortens it, or the real seconds track game speed** — either would mean a removal path exists that the source read missed, and the entry must be corrected |

| **C26 stranded-maintenance dump** ⭐ **ADDED 2026-08-02 (prompt 6c) — one console line, no setup, but it needs a HEAVILY LOADED save and that is the whole point.** ✅ **READING 1 of 2 TAKEN 2026-08-02 — CLEAN** (`10 … 0 structurally broken`, sol 288, log `Mars.exe-20260802-01.31.10:225`). **STILL OPEN: take reading 2 on a DIFFERENT colony**, not this save again — a different history is what the second sample is for. **Reading 1's colony was founded on the pinned build itself** (`OrigLuaRev` = `LuaRevision` = 396349), so the fixups never ran there and its 288 sols are unhealed live evidence. ⛔ **A second dump WAS taken 2026-08-02 and does NOT count** — it was a 98-sol save of the **same lineage** (`save_game_id: HdmSxGs6kyd0uz6-`, same sponsor/profile/coordinates/map), so its history is a subset of reading 1's. ⭐ **Before counting any future dump, check the log's `more_game_settings:` line and confirm `save_game_id` differs from `HdmSxGs6kyd0uz6-`.** The vintage question is now CLOSED for that lineage (`OrigLuaRev` = `LuaRevision` = 396349 **and** `UndergroundRework106 = true`, two mechanisms with opposite failure modes agreeing), so reading 2 only needs the dump plus the id line | **Why this is a dump and not a watch:** the C26 sweep could not find a producer in current Src, but the engine ships two savegame heals named for exactly this state, so **the broken state is directly observable rather than derivable** — you look for it, you do not wait for it. **Take it on the oldest / most built-up colony available, ideally at the END of a long session** (a fresh save proves nothing; the fixups only ever ran on old saves, so anything found here is the producer firing live on our build). One line, underground map included: `*r local n, bad = 0, 0 AllMapsForEach(true, "RequiresMaintenance", function(o) if not o:DoesRequireMaintenance() then return end if not (o.maintenance_phase or o.is_malfunctioned) then return end n = n + 1 local w = o.maintenance_work_request local why = false if not w then why = "NO work request" elseif not table.find(o.task_requests, w) then why = "work req NOT in task_requests" elseif w:GetSource() ~= o then why = "work req source mismatch" elseif o.ui_working and w:IsAnyFlagSet(const.rfSuspended) then why = "work req SUSPENDED while building is ON" end if why then bad = bad + 1 ConsolePrint(o.class .. " -- " .. why) end end) ConsolePrint(n .. " buildings in maintenance or malfunction, " .. bad .. " structurally broken")` **Reading: `bad = 0` is the expected and useful result — record it, it is evidence the producer is gone in Relaunched and C26 can be closed after a couple of clean dumps.** Any non-zero line is the defect caught live and C26 earns an F-row immediately; **note which of the four reasons printed**, because each names a different producer. ⚠️ **A building sitting in `demand` phase with no maintenance resource in the colony is NORMAL and will be counted in `n`** — only `bad` matters. Decides C26 (BUGS C26 entry) |
...
**F82** a grid split). **C20 (added 2026-08-02) waits for nothing** — it needs
only a probe and an unexplored sector, and it is two minutes. **C25 (same
date) waits for its situation** — a Jumbo Cave Reinforcement site that sticks
on "construction site is being cleared". **C26 (added 2026-08-02) waits for
nothing either, but it wants the OLDEST save you have** — it is one console
line, and a clean result on a heavily loaded colony is the result that closes
the entry.
