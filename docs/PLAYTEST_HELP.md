...
| `CheatDustDevil(major, setting)` | `Lua/Cheats.lua:47` | dust devil at the camera look-at. ⭐ **A STATIC-CHARGED devil can be forced by deleting the lottery** (source-verified 2026-08-04, `[NEVER RUN]`): the electro roll is `Random(100) < descr.electro_chance` on a plain preset table (`DustDevils.lua:138`), so drive the shipped site with a copied descr: `*r local d = table.copy(Presets.MapSettings.DustDevils[CurrentMap.mapdata.MapSettings_DustDevils] or Presets.MapSettings.DustDevils["DustDevils_VeryLow"]) d.electro_chance = 100 local dev = GenerateDustDevilIn(GetTerrainCursorClamped(), CurrentMap, d) if dev then dev:Start() end` — aim with the mouse; do NOT use `GetCameraLookAtPassable` (file-local, invisible from console — see the meteor row) |
