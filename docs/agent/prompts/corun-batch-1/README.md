...
| 2 | ~~`02_OPUS_SITTING.md`~~ **CONSUMED 2026-08-05** | Opus | **YES — it ran, and it cost ~2 h against ~24 min promised** | ⚠️ **PARTIAL.** RAN: leg 1 (PT-37 — case A **PASS**, better than a no-op; case B **UNSAMPLED**, the decider settled the other way and F48's blocking premise contradicted for its cited scenario), leg 3 gate (**SKIP** re-confirmed live), leg 4's in-dome half (**drain MEASURED** 76→37). ⛔ **NEVER RAN: PT-47, popup trio + F85, PT-35, and PT-53's uninstall half** — all routed. Out-of-scope but owner-driven: **F99 two witnessed negatives** + ⭐ **new `F101`** (5 witnessed throws) + a `D12` self-check defect. 8-entry ledger, 4 owner decisions routed to the checklist. Evidence: `docs/archive/cb1sitting_Mars.exe-20260805-14.28.49.log` |
