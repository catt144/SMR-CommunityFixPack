...
| ~~99~~ | ~~`99_TERMINAL_AUDIT.md`~~ | Fable | ✅ raised three | ✅ **DONE 2026-09-09** (`smr-bugfixpack-a8`): **SHIP WITH CHANGES** — `reports/HOTFIX_2_AUDIT.md`. Instruments re-run and falsified by hand; every FIX-set body three-way diffed and holding; Pass D re-read every KEEP wrapper on a changed target (58 branch diffs, four fresh readers, every finding re-opened on the shipped lines): **F117** `ArrivalDeaths` THROWS on 1.1.0 (`ChooseDome` takes the colonist now; P1, source-read, live in v5) ⇒ ck127 · **F118** `LayoutTechLock` clears 1.1.0's delete-on-load registration · **F60** REMOVE candidate (the gate no longer reads the tally) · **C55** vanilla pre-sort assert on repair-site tracks (F45 KEEP confirmed). Pass F: strings vs both backups byte-identical by script, but the change note's bullet 2 contradicts link 08's pass ⇒ ck128 rides ck126 (06 ran before 08). Site publish WITH the upload ⇒ ck129. Pass G: NO boot of the 44-module pack exists; prediction 44/0. ⛔ Nothing ran in a game; no status moved; no code edited. Authored `100_DOCSWEEP.md` as its last act. ⚠️ **`99` STAYS in the folder per its own §13** (SHIP WITH CHANGES ⇒ the chain is not over): its §5 table is the inbox of the code link decision **127(a)** would create, and that link's author consumes and `git rm`s it; if **127(b)** is ruled, `100` deletes it on its own close-out. A folder holding `99` + `100` + this file is the designed state until then |
| 99a | `99a_F117_FIX.md` | any | no | ⏳ **AUTHORED 2026-09-09 by the on-call session (`smr-bugfixpack-77`) under owner decision 127(a) (`ab7e0f5`); NOT FIRED.** The changes 99's SHIP WITH CHANGES demanded, and the last code that goes into hotfix 2: **F117** (`Fix_ArrivalDeaths.lua:201` passes the colonist on the 1.1.0 body behind a per-module runtime discriminator — ⛔ ck118 binds, `self` on both branches unguarded would MIS-SCORE every dome on 1.0.7, which is worse than the throw), plus two one-line riders — **F118** (`Fix_LayoutTechLock` re-asserts `s_ConstructionControllerDeleteOnLoad`) and a named-decline `Require` guard in `Fix_PayloadTemplateRefill` — plus one Test Kit probe. ⚠️ Kit lane is NOT free: `SELFCHECK_PILOT.md` also writes one kit file; sequence with it. Inbox: `reports/HOTFIX_2_AUDIT.md` §1 F-1/F-2/F-6 + §5 (copied into the prompt, since it deletes `99`). ⛔ Adds NO `Code/*.lua` file ⇒ `items.lua`/`metadata.lua` stay shut. **Runs BEFORE `100`** — the sweep's §3.2 keys off this link's commit. On close-out it `git rm`s **itself and `99_TERMINAL_AUDIT.md`** |
