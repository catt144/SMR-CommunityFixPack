...
| 99b | `99b_BENCH.md` | Opus | no | ⏳ **AUTHORED 2026-09-09 (evening) by `smr-bugfixpack-86`; NOT FIRED.** The bench link — everything owed before the doc sweep that does **not** need a game. Drains the three items the owner ruled during sitting 2 (`1851b1a`): **Unit A** repair the four stale instruments (the `CaveInRubble`/`IsNearDome` stub gap that prints a real `[LUA ERROR]` header into every log, `LandscapeCostGuard`, `LanderReturnFuel`, `SaveSanitizerUpgradeLeak`) · **Unit B** build probes for `LanderEmptyLaunch` + `FreedHousingNotice`, the two shipped modules with no working coverage · **Unit C** the `Code/` comment corrections `100_DOCSWEEP` §4 handed to “decision 127's link”, which has since fired and closed, leaving them orphaned · **Unit D** correct `bugs/F117.md` §Control, whose recipe sitting 2 refuted from the shipped source, and its unsupported “ordinary mid-game” frequency claim · **Unit E** surface the `STATE.md` headroom question. ⛔ **Out of scope by owner instruction: the repaired-but-never-observed pair (F117/F118).** No game is launched, no status word moves, `Code/` gains no module. Fires BEFORE 100 |
...
> its own session (99 by `aac8e9c`, 99a by its close-out below). One link is
...
> change-note line must not say "Fixed". Two extra things to carry: the routed
> stale-1.0.7 instance at the bottom of this file, and checklist **131** (whether
> the F117 desk falsifiers get promoted out of a scratchpad into `tools/`).
...
13. **A doccheck WARN goes verbatim into your summary.** `STATE.md` is
    ...
    because items were going unrecorded). ⚠️ **Headroom re-measured after 99a:
    11456 bytes used, ~830 left — about 8 lines, not 32.** A line you add still
    fits, but the next link should expect the warn rather than assume room. It is still a kernel: add the fact, not the story, and
