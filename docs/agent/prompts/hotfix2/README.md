...
| ~~100~~ | ~~`100_DOCSWEEP.md`~~ | Opus (fired on Fable) | ✅ 126 + 127 RULED `ab7e0f5` | ✅ **DONE 2026-09-09** (`smr-bugfixpack-05`): the inverted sweep, docs only, nothing ran in a game. **Unit A** `7830134` — `last_changes` bullet 2 takes the ck126 KEEP wording ("removed the next time that save loads", `F95`) and bullet 4 gains the F117 line in the RE-DERIVED recipe's words (ck127(a), `777249d`); `metadata.lua` + `UPLOAD_WORKFLOW` §3 + `STORE_CARD_LIVE` in ONE commit, byte-identical by script (description 5342 == §3 plain, BBCode == BBCode, note == note, summary == summary, SCL plain +143 = the two portal passages), parsecheck OK, preflight 0 FAIL. **Unit B** site `dc892d1` — the two "built against 1.0.7.396349" lines → 1.1.0.403908 (`EF-078`) with the frozen build named; FAQ save-repair list gains the F95 pass; ⭐ `for-modders.md` named `DustDevilSpawnGate` (deleted) → `LakeEntombment` — a retired-ID hit the phrase sweep cannot see; ⭐ `index.md`'s live "Nothing here is published yet" note removed; `mkdocs build --strict` clean; published AFTER the upload (ck129). **Unit C** `5d6af93` — §4's drift list (README archive paragraph, template count-agnostic, `POST_UPLOAD_CLOSE` "82 entries" = the v1.0.0 PACK's entry count → `pack_predict.py`, `WORKFLOW:407` VOID, `F115` "17 as read (16 heal-aware)"). **Unit D** = the commit that strikes this row: checklist receipts for 126/127/128 + the 129 order, **ck133** (six §7 decisions routed by `-db`), STATE, SESSION_LOG, this prompt `git rm`'d. ⚠️ Drift: three seeds stale on arrival (EF-075 for the build number, "82 entries" as a fix count, README:137 → :164); `EF-075`'s summary still says the 1.0.7 base is gone — FILED. Retired-phrase sweep: 0 hits anywhere. ⛔ No status word moved; `version` untouched; NOT clearance (`H-04`) |

> ✅ **THE FOLDER NOW HOLDS THIS FILE ALONE — THE CHAIN IS CLOSED (row 100 struck
> 2026-09-09).** Every prompt has been consumed by its own session (99 by
> `aac8e9c`; 99a, 99b and 100 by their close-outs). Nothing is left but the upload
> sitting, in ck129's order: upload → store pages → publish the site.
>
...
module deletion lands in 02. Reason: `module-list gate (tools/doccheck.py MODULE SETS + tools/upload_preflight.py)` inverts on this patch — with 37
...
12. **Bindings in force:** `editor/version rail (agent/prompts/perma/RELEASE.md § Release rails)` (no Mod Editor, no `version` edit, no upload —
    those are the owner's sitting), `H-03`, `H-04`, `H-08`, `H-09`, `module-list gate (tools/doccheck.py MODULE SETS + tools/upload_preflight.py)`.
