...
- ⭐ **19 orphan research icons name techs that exist nowhere in the tree**, and they have
  **no loc strings** — art-only debris, the opposite shape to this tech.
...
⛔ **A cross-version layout diff is IMPOSSIBLE and must not be attempted.** 1.0.7 has no
hex tree at all — its techs are `PlaceObj('TechPreset', …)` in `Data/TechPreset.lua` with
**no `MapPos` anywhere**. The `Tech` class, the hex grid and the group rings are all new
in 1.1.0. There is no prior layout to diff against, which is *why* this is hard.
