...
|---|---|---|---|---|
| 01 | `01_SPEC_fable.md` | Fable | routes decisions | re-validates this cut + placement; re-derives the routes; a disposition for every currently registered module; links; Beta model; surface package; §3a for runtime switching; policy text |
...
| 08 | `08_TEXT_AND_SURFACES_opus.md` | Opus | no | player titles/descriptions for every row; `items.lua`/`metadata.lua`; store backups; site + FAQ source; TestKit + current support/policy docs |
...
Model placement: the top tier sits on 01 (the spec — a wrong contract poisons every conversion), 06 (save-state code, the
...
The module names and allocations in links 04–06b are the `889faab` authoring
snapshot, not today's inventory. Link 01 must emit the current registered set,
reconcile it to `items.lua`/`metadata.lua`, and assign every member exactly once.
An unassigned or duplicate current module is a stop; no later link silently
inherits the old total.
...
- **Terminal lifecycle:** after either form of 99 closes, archive this README to
  `docs/archive/prompts/fixtoggles/README.md`, remove the `fixtoggles/` row from the prompt map, and leave no live
  `docs/agent/prompts/fixtoggles/` directory. The archive destination must be new; never overwrite an archived file.

## Binding chain rules — every prompt inherits these

1. **Staleness check first.** The chain was authored at `889faab`. Run `git log --oneline -10`, `git pull`,
   `git status --short`, compare each link's named inputs from `889faab..HEAD`, and re-check moved groups. `ListAgents` may
   identify Claude peers, but not Codex. Several sessions
...
`docs/agent/STATE.md` (called for by this chain) · `docs/agent/reports/FIXTOGGLES_RESEARCH_2026-09-11.md` · `docs/agent/reports/FIXTOGGLES_SPEC.md`
...
`B:\Dev\SMR\SMR-Shared\SMR-SrcArchive\1.0.7.396349\Src`. Working model of the Mod Options route: `B:\Dev\SMR\SMR-OptInPack` (`items.lua`,
...
|---|---|---|
| this live folder has one README and 13 fireable prompt bodies | enumerated at the leg-06 audit on `4cc0d4c` | recursively enumerate files before a link starts and reconcile each name with the queue |
