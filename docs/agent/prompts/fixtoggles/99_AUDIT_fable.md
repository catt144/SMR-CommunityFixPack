# fixtoggles 99 — the terminal adversarial audit

Link 99 of `fixtoggles`. Fresh context; your job is to **disbelieve the chain**. README binding rules 1–18 are yours.
...
3. **Re-derive, sample, never inherit:**
   - the spec's routes (store, surface, gate, blacklist list) against the shipped `Mod.lua`;
   - at least 12 converted modules read line by line against their shipped bodies — all of 06/06b's, the `Colonist:Idle`
     pair, three of 04's, three of 05's — each: off ⇒ the captured original untouched; on ⇒ unchanged from before the
     chain (diff against the pre-chain commit);
   - every harness assertion's falsifier re-run RED by you;
   - 03's and 11's verdicts against the ARCHIVED logs, prediction by prediction;
   - every player string (row text, store backups, site source) against rule 8 and rule 9, by script where possible.
...
Anything you did not re-derive or re-read yourself. "Release ready" (`H-04`).
