...
| 2 | **every module applies** — ⛔ *sharpened 2026-08-19: this said "registers", which is a weaker and different event. A module can register and then latch `inactive`; registration proves nothing about the fix working.* | **75 `applied` witness lines BY NAME**, ⛔ never a total. ⚠️ **Do not phrase this as "equals run A's set"** — that made the gate depend on a run that may not have happened, and a gate must not have a dangling referent. The concrete expectation is the 75 ids in `metadata.lua`'s `code` list minus `00_Core`/`90_SaveSanitizer`. ⭐ **One known-benign exception, pre-registered so it is not mistaken for a failure:** `SaintBlessing` runs `applied` → `inactive (no dome-colonists trait presets)` → `corrected N of M` on **every** launch (62/59/58 across the archive). That cycle is expected; its *absence* would be the anomaly |
