...
| `PLAYTEST_HELP_DISSOLUTION.md` | ⭐ **LIVE, NOT FIRED — authored 2026-09-15, ck182 RULED.** Dissolves `docs/PLAYTEST_HELP.md` (62,965 B) across the four destinations ck182 names — hazards and the 5a line into `prompt-authoring`, Test Kit helpers into the SMRTK reference, the co-run rig into `perma/CO_RUNS.md`, and the command reference, save fixtures and archived-`TESTING.md` list cut. ⛔ **The four destinations do NOT cover the file:** ~17,349 B across `Console: what works and what silently does nothing` (9,402 B), `The ENABLE-PATH leg`, `The MarsDebug [install] pass` and `Harness quick facts` have **no destination in the ruling**, and stage B exists to home them — treating the four as exhaustive cuts unhomed content. ⚠️ The deletion is **gated**: `CLAUDE.md:30`, `docs/README.md:13` and `tools/doccheck.py:125` all name the file and must change in the same commit, and the corpus's canonical rule count drops from 32. Carries `PENDING_MOVES` row 3 (the warmed-up-save rider line), deferred here deliberately by the skills brief. |
