...
| `MOVE_SHARED_high.md` | `prompt` | ⭐ **LIVE, NOT FIRED — fires BEFORE `MOVE_FIXPACK_high.md`** (owner, 09-21). Creates `SMR-Shared` at the new root and moves the archived game source and the Workshop corpus into it, so the fix pack's five tool consumers are repointed once and the fix pack is opened once. Pull-only, no remote, no scaffolding; source-line citations are records and the map's Path translation block does the translating. |
| `MOVE_FIXPACK_high.md` | `prompt` | ⭐ **LIVE, NOT FIRED** — second pass of the `B:\Dev\SMR` move: this repo, the TestKit and `SMR-ScreenCaptures` (the one cleared non-repo at that root), together because the tooling defaults point at a `C:\Dev` sibling. Fired from a session rooted in `B:\Dev\SMR\SMR-OptInPack` (owner, 09-21) so the mover sits outside every tree it moves; the `saves/` symlinks must arrive as links, not copies of the owner's live saves; `core.hooksPath` is absolute and would silently stop gating; the Claude memory store follows the root path. |
| `MOVE_AUDIT_high.md` | `prompt` | ⭐ **LIVE, NOT FIRED — held until the LAST move pass reports done** (owner, 2026-09-21: one audit of the whole move, none per pass). The adversarial grade, by a seat that did no pass; the moved opt-in trees are live by then, so it attributes differences by commit. Report at `reports/MOVE_AUDIT_<date>.md`; it grades, never repairs. |
