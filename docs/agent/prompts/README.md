...
| `WILDFIRE_CURE_RESEARCH.md` | `prompt` | **LIVE, NOT FIRED — authored 2026-09-16** (owner ask): a player reports no option to research the Wildfire cure in the new tech tree. Deliberately wide licence — explore freely, launch the game, find **cause and fix end to end**, and reach confidence **without** booking an owner playtest if it can be earned honestly. Carries 13 established facts with falsifiers and **four already-refuted hypotheses** so none is re-run, including that the chain rewiring is complete (5 chained families = 5 remapping entries) and that the dead `"In Progress"` research state is used exactly once tree-wide. Routes findings that belong to [C69](../bugs/C69.md), [C79](../bugs/C79.md), [C92](../bugs/C92.md) or [C97](../bugs/C97.md) to those entries, and asks whether the cause is a **class** spanning them. ⛔ Asking the reporter for a save is out of bounds. |
