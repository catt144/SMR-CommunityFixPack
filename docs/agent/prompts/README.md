...
| `STILL_NEEDED_SWEEP.md` | 🚧 **READY, not started (09-12)**: per-module settlement of "is this fix still needed on 1.1.0?" — retire / rebuild / keep, with the fix-list row judged separately. Prompted by two PDX devs planning hotfixes off our list. **Written for a FAN-OUT coordinator** (one module per agent, fixed schema, hazards + serial bottleneck named); fired on Codex/Astra 09-12; ck156 |
