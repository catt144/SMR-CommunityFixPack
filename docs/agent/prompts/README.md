...
| `C93_RANCH_DRONE_BUILD.md` | `prompt` | ⭐ **LIVE, NOT FIRED — authored 2026-09-16 from the owner's own reproducing save.** The Outside Ranch drone stall: 6472 log errors of one shape only, all `HGE::l_HasSpot: The object given has no entity` on the drone approach path, resuming 0.4 s after load. Production works; collection does not. **Leg A names the entity-less object before any fix is designed** — the desk narrowed it and could not close it. Carries a correction to [C93](../bugs/C93.md)'s control table (its "log line absent ⇒ refuted" test is too strong: the pasture fixup's `Origin` fallback is silent). Defect record [C93](../bugs/C93.md). |
