...
| `C95_REPOINT_BUILD.md` | `prompt` | ⛔ **LIVE, NOT FIRED — authored 2026-09-16 after an attended sitting found the shipped `Fix_HabitatExpeditionDraft` INERT.** It hooks `CargoTransporter.GatherAvailableColonists`; 1.1.0 expeditions run through `UniversalRocketBase` + `CargoTransporterNew`, so the receiver guard never matches and the armed `CrewDraft` logger — which wraps the same function — stayed silent through a real launch. The job is to **move** the existing repair, not rewrite it: §2 makes the receiver-coverage decision explicit, §3 carries the no-yield re-check the new path needs. Records [C95](../bugs/C95.md) · [sitting](../reports/C95_SITTING_20260916.md). Checklist 189 is NOT satisfied. |
