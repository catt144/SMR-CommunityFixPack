...
| `C95_HABITAT_DRAFT_BUILD.md` | `prompt` | ⭐ **LIVE and AUTHORISED — ready to fire.** Rewritten 2026-09-16 when the owner lifted [ck185](../../PLAYTEST_CHECKLIST.md)'s C95 half (*"I want to fire it and then we can test it, already have the test ready in a save"*); ⛔ **C96's half stays open**, and the release itself is still the release prompt's job. Builds the ruled repair — exclude habitat residents from the **expedition** draft, ⛔ not a return-path widening — as a **main-pack judgment call with the mark**. Carries the design the owner worked out with it: predicate on `MicroGHabitatBase` (⛔ the leaf class misses the Micro-G habitat), hook wrapping the per-bucket `FilterColonistsByTrait` **scoped to the picker call**, which is what keeps the lander and elevator out. ⛔ Records two shapes that do NOT work and why — filtering the returned list hangs the rocket forever, and the residency flags are player-toggleable dome policies. Owner has the test save; ⛔ do not provision one. |
