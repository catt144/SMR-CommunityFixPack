...
| `REPAIR_PASS.md` | ⭐ **NOT FIRED — for Codex.** Six defects that are wrong under any answer to the open policy calls, in three independently committable groups: marker integrity reported instead of silently dropped (warn, NOT red — `part-ruled` and two duplicate `ck` numbers exist now), the fingerprint `HOLDS` verdict decided by substring membership, dead `prose_defer`, `.rgignore` shipping to players + an ignore-list parity gate, the marker obligation homed at the authoring entry, retirement banners on three retired-but-firable prompts, and three shipped comments left reading as machine output. Every gate it adds must be shown to FAIL on a broken copy. |
| `REPAIR_PASS_C.md` | ⭐ **NOT FIRED — for Codex.** Finishes `REPAIR_PASS` group C, which halted on one refuted sub-item (the pilot was already deleted) and took three unrelated repairs with it — the halt was the brief's fault and the stop rule is corrected at the top: **a refuted defect is a finding, skip it and continue.** Part 1 (no ruling needed): the marker obligation homed at last, `IMPLEMENT_PROMPT`'s live firing route banner'd, a false claim about the deleted pilot, three shipped comments re-wrapped. Part 2 fires ONLY if the owner has ruled ck170(a) — carries the proposal, including duplicate `ck` numbers as WARN-when-agreeing / RED-when-disagreeing, which closes ck144 without renumbering. |
