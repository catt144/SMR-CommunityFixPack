...
| `HAZARD_KERNEL_PASS.md` | ⭐ **NOT FIRED — for Codex.** Executes the owner's 09-13 rulings on STATE's kernel: the three-part admission test (harm floor · universality · gate) into `STATE_EVICTION.md`, H-10/H-01/H-02/H-07 out to their gates, H-06 out on a failed harm test (EF-056 keeps the fact), H-04 moved to Rules in force, and a second phase collapsing seven restated rules + the open-decisions enumeration. Two independently committable phases; §6 handover table is written FOR an adjudicating session, which is a different seat by owner ruling. |
