...
| `RULES_HEADERS.md` | ⭐ **LIVE, NOT FIRED — for Codex.** The owner's 2026-09-14 design ruling: **N scattered rules become N local header blocks plus ONE kernel rule**, so a session that never opens a doc never carries that doc's rules. Inventory every rule, classify each as doc-local / task-local / global / redundant / **dead**, then an **OWNER GATE** before anything moves, then land the headers (byte-capped, one commit per source doc), then the single `STATE.md` line and a structural doccheck check. ⚠️ Carries the definition that keeps it tractable: the checklist's **319 `⛔` are 317 records and 2 rules** — an inventory that counts every `⛔` is useless. ⚠️ `STATE.md` has ~112 B of headroom, so the kernel line is only affordable if the audit finds doc-local rules to evict from it. ⛔ Must not fold in checklist **177**'s open marker gate. |
...
- `smrtk/`: ⭐ **LIVE** (authored 2026-09-13, owner ask, checklist 175): the **SMR Tool Kit** — a TestKit panel that
  supplies Selected/World playtest actions through untainted leaves (`EF-095`), one `SMRTK_` log tag, agent
  ...
  **03B JUDGED PASS WITH FIXES 09-13** (`reports/SMRTK_JUDGE.md`); the owner then ruled ck175 item 1 on 09-14
  ...
  `reports/SMRTK_FULL_SITTING.md` + **ck183**; its prompt is retired. So next is owner `08b_SURFACE_SITTING_owner.md` (09's build/desk complete;
  changed-surface acceptance, gated on owner surface stability; ⛔ the Stamper was CUT 2026-09-14 and class 18 is struck),
