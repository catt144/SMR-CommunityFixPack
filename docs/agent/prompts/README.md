...
| prompt | state |
|---|---|
| **`VANILLA_DIFF_DISPOSITION.md`** | ⭐⭐ **UNFIRED — WRITTEN 2026-09-12 (`7806c0a`), READY TO TAKE.** ⚠️ Most rows in this table are **graves** (struck through = already fired); **this one is not** — nobody has run it. Tool-neutral, read-mostly: what we do with the vanilla diff, how far we trust the four instruments, and where that information should live. ⭐ Its urgent half is a **live procedure gap** — `WORKFLOW.md`'s two after-every-patch rules name none of `treediff`/`presetdiff`/`bodycheck`/`sigcheck`, while the store card now publishes a recurring-process claim. Also asks for the **2 NO-MANIFEST** modules to be named. Deliverable: `agent/reports/VANILLA_DIFF_DISPOSITION.md` + a checklist item; `git rm` this brief and grave the row in the landing commit. |
