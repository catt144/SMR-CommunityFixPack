...
| `DOC_EDITING_SKILLS.md` | ⭐ **LIVE, NOT FIRED — authored 2026-09-15.** Builds `doc-editing` and `prompt-authoring`, but **the first deliverable is an audit, not a skill**: `reports/DOC_RULES_ARCHITECTURE.md` §5's runbook list predates several of its items becoming doccheck gates, and restating a gated duty as skill prose is redundancy in a new container. Stage A asks of every item *"is this already gated?"*, verified against `tools/doccheck.py` output rather than a check's name, then sorts the remainder into GATE / SKILL / DROP. ⛔ **Read `.claude/PENDING_MOVES.md` FIRST** — it is gitignored, nothing points at it, and its rows are silently lost if the builder skips it; the drain is stage D and a half-move is worse than none. ⚠️ Building `doc-editing` makes `CLAUDE.md`'s *"invoke the doc-editing skill"* rule compliable for the first time. ⛔ The ck182 `PLAYTEST_HELP` dissolution is explicitly OUT — it is the owner's open call. Caps are reported and proposed, never set. |
