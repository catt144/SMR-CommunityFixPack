...
| `WILDFIRE_CURE_INVESTIGATION_high.md` | `prompt` | **LIVE, authored 2026-09-18** (owner ask, Reddit report): does 1.1.0's at-home service route pay medical stats without the Wildfire cure's `Service` hook, so the mystery cannot finish? Investigate, file, and build if clear-cut. |
