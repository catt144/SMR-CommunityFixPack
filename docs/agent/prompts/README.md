...
| prompt | state |
|---|---|
| **`SITTING_158.md`** | ⭐⭐ **UNFIRED — WRITTEN 2026-09-12, READY TO RUN.** The **ck158 sitting**, which is **the v10 gate**: C85, C89 (the ⚖️ judgment call the owner asked to watch) and C88 — built, desk-checked, **none run in a game**. ONE boot does all three. A live playtest with the owner at the keyboard; the steps stay in checklist 158 and are pasted from there, while this prompt carries the stale-probe gate, the hazards, the pass artefact per leg and what to do with the result. ⛔ `tested-attended` is the owner's word to grant. `git rm` when fired. |
