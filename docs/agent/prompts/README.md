...
| `SITE_ALIGNMENT_AUDIT.md` | ⭐ **LIVE, fire before the held site deploy** (written 09-13 by `smr-bugfixpack-c2`): audit **and fix** every player-facing promise orphaned by a retirement. Root cause: `PUBLIC_SURFACE_SWEEP` §1 only ever checks what a **new** fix falsifies, never what a **retired** one stops doing — so **40 module deletions since hotfix 2** have never been swept, and the one that was checked (`faq.md`'s "phantom farm oxygen") was wrong. Carries the two-place rule for live store cards and a hunk-split fence around the owner's 3 unruled pared files. ⛔ Never fires the deploy. |
