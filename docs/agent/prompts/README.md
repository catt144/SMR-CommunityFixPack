...
| `CHECKLIST_ARCHIVE.md` | ⭐ **LIVE, NOT FIRED — for Codex.** Mark the settled-but-unmarked backlog in `PLAYTEST_CHECKLIST.md` and archive it: **35 items / 159,621 B** qualify on top of the 16 that already move, ~27% of a 741 KB file. Four commits, with the **date-only** half kept separately revertible. ⚠️ **34 of the 35 are UNNUMBERED, so rule (d) cannot protect them by construction** — that risk is the reason for the split and for the Sonnet recheck that follows. `--apply` has never executed; it is falsified on a copy first. From checklist [176](../../PLAYTEST_CHECKLIST.md). |
...
- `smrtk/`: ⭐ **LIVE** (authored 2026-09-13, owner ask, checklist 175): the **SMR Tool Kit** — a TestKit panel that
  ...
  **Codex builds (01, 03A fan-out, 07), Claude judges (03B) and audits (99)** — the owner's shape. 01 built;
  next is the owner's `02_SKELETON_SITTING_owner.md`, with Claude attending. Its README is the manifest; `payloads/` holds 03A's briefs.
