...
| `C92_PLACEMENT.md` | **LIVE, authored 2026-09-13** (owner ask): C92's four open questions — should we fix it, which icon, what the tech data says about its intended home, and where the hole in the web is. Evidence base = `reports/C92_INVESTIGATION.md` "Addendum 2026-09-13" (accident verdict on five controls; `UnlockTech` finishes the wiring in one call; loc complete in 8 languages; **no research icon exists**, 19 orphan ones do; no cross-version layout diff is possible). ⭐ Owner's bar on placement is **evidence, not inference**. |
