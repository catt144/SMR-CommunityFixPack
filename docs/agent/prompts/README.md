...
| `DOC_OVERHAUL_AUDIT.md` | ⭐ **NOT FIRED** — backward QA on the context-economics doc/process overhaul (`1a487c0..0c6fd1f`): 13 claims with their falsifiers, plus an explicit licence to name what the overhaul should have covered and did not. Report-only; fresh session, any model. Seeded with two verified defects (`status:part-ruled` is invisible to `MARKER_RE`; duplicate `ck` numbers let a collapsed heading win its register row) and one open orphan (the marker obligation lives only in `perma/HANDOFF_ORCHESTRATOR.md`). |
