...
| ~~`REPAIR_PASS.md`~~ | **FIRED 2026-09-13** (Codex). Groups A/B landed in `424075c` and `03fc504`; Group C's initial stop is recorded in `fd54c7e` and its follow-up repairs in `9e4691d`. [Handover](../reports/REPAIR_PASS.md) carries falsifiers, emitted results and remaining scope. Brief consumed after Group C landed. |
| ~~`REPAIR_PASS_C.md`~~ | **FIRED 2026-09-13, Part 1 complete** (Codex, `9e4691d`). Marker obligation homed in WORKFLOW, ignored IMPLEMENT prompt marked spent, handoff retired and pilot removal corrected, Lua comments rewrapped. **Part 2 remains held on the owner's ck170(a) ruling**; markers and thresholds unchanged. [Handover](../reports/REPAIR_PASS.md) preserves the deferred recipe's history route. Brief consumed. |
| `CK170_AND_FINGERPRINTS.md` | ⭐ **NOT FIRED — for Codex.** Phase 1 records the owner's 09-13 ck170 rulings (keep the four marker words · normalise LF bytes · cite the rails, kill DISPATCH step 3's 97 KB index scan, file R-D as an authoring rule) and closes the item. Phase 2 is the real prize: **55 of 93 facts read MOVED and 54 of those carry an INFERRED date**, so the verdict forcing re-derivation on ~59% of the fact base rests on a guess — shrink it with evidence, conservatively, because a false HOLDS silently licenses skipping evidence. Goal-stated, not scripted: Codex may alter the approach or stop and ask. |
