...
| `MOVE_REST_high.md` | `prompt` | ⭐ **LIVE, NOT FIRED — fires BEFORE `MOVE_FIXPACK_high.md`** (owner, 09-21). Everything left but the fix pack, in one pass: new `SMR-Shared` (game source + Workshop corpus) and new local-only `SMR-FR1`, plus the two community repos. One pass so the fix pack's map and tools are repointed once before it moves. Records translate via the map; `LINUX_DISPATCH.md`'s "outside git" wording is repaired here. |
