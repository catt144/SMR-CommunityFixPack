...
| `MOVE_OPTIN_high.md` | `prompt` | ⭐ **LIVE, NOT FIRED** — first pass of the `B:\Dev\SMR` move (owner, 2026-09-21): the opt-in pack and `SMR-Assets`, ahead of every other tree because the train hub build is on hold until they land. Copy-verify-rename, never delete; the train orchestrator owns the symlinks, the editor-held paths and `tools/devmods/**` contents. |
| `MOVE_FIXPACK_high.md` | `prompt` | ⭐ **LIVE, NOT FIRED** — second pass of the `B:\Dev\SMR` move: this repo, the TestKit and `SMR-ScreenCaptures` (the one cleared non-repo at that root), together because the tooling defaults point at a `C:\Dev` sibling. The mover is rooted in the tree it moves; the `saves/` symlinks must arrive as links; the Claude memory store follows the root path. |
