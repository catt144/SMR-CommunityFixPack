...
| `STANDDOWN_AUDIT.md` | `prompt` | **LIVE, authored 2026-09-13** (owner ask): can full-body replacements stand down when vanilla fixes a defect? The old 21-of-45 split is an explicitly re-derived seed, not a current total; scoped 2026-09-19 to the runtime decline for players (the desk class-c detector is `GAME_PATCH_BUILD_high.md`'s D1). Design record [D14](../bugs/D14.md). |
| `GAME_PATCH_BUILD_high.md` | `prompt` | live one-off build, owner-ruled 2026-09-19: `tools/patchcheck.py` with its 1.1.0 backtest as regression test, the perma `GAME_PATCH_PROMPT.md`, and the opt-in fork's `gamepatch/` outbox; design `reports/GAME_PATCH_INSTRUMENTS.md` |
