...
| `GATE_WIRING.md` | ⭐ **LIVE, NOT FIRED — for Codex.** Two independent repairs in one pass: wire `ck170_selftest.py` and `repair_pass_selftest.py`, which nothing runs, and make STATE's `BUILD STATE` block regenerated rather than hand-typed. ⛔ Item B is a **`--regen` region, not a comparison gate** — a bare gate would have blocked 26 entry commits since 09-01, while regeneration is free because a stale `bugs/INDEX.md` is already RED. A stop on one item does not halt the other. Finding: [the adjudication](../reports/CK170_AND_FINGERPRINTS.md). |
