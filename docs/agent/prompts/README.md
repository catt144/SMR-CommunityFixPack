...
| `C95_RETURN_HOME_BUILD.md` | `prompt` | **LIVE, authored 2026-09-17** (owner ask): BUILD, not research. Habitat residents return to the expedition draft and come home through the audited selector seam; C102's fallback becomes the nearest safe dome (owner ruling 09-17). Lands in `Code/` with desk and unattended live evidence, appends the outbox entry, hands the owner a sitting; no upload |
