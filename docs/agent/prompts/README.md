...
| `F120_AUDIT.md` | `prompt` | **LIVE, NOT FIRED — authored 2026-09-16** (owner ask): cross-vendor audit of the Wildfire fix, to be fired on **Fable** because **Astra built it**. Same wide licence as the investigation — explore freely, launch the game, and it **may expand or reshape the fix**. Leads with the owner's actual question, did it miss anything: whether a converter that drops `field='Mysteries'` wholesale leaves the **other ten mysteries** with the same hole while the fix is gated to Wildfire alone; whether it reaches the reporter at all (F120 says it does not); and whether its repair contract survives testing rather than assertion. ⚠️ The module is **registered and would ship**, so a must-not-ship finding is required to come first and loudly. ⛔ Asking the reporter for anything is out of bounds. |
