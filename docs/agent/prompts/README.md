...
| `C90_GUARDS_BUILD.md` | ⭐ **LIVE, on the v10 path** (written 09-12 by `smr-bugfixpack-c2`): build the per-module apply-success guards in `Fix_SaintBlessing` + `Fix_SinkholeIndestructible`. Owner ruled 09-12 that **v10 carries C90**; shape was already ruled at checklist 158. ⛔ NOT the shared core, ⛔ never gate on `entry.status == "active"`. Carries a lane fence — a sibling session lands the three retirements in parallel. Ships **unexercised** (`fixed`, never `tested-attended`), **no public row**. |
