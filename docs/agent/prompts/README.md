...
| `C92_PLACEMENT.md` | **LIVE, authored 2026-09-13** (owner ask): C92's open questions — should we fix it, which icon, what the tech data says about its intended home, where the hole in the web is, and the residue risk if we finish the work rather than bypass it. Evidence base = `reports/C92_INVESTIGATION.md` "Addendum 2026-09-13" (accident verdict on five controls; `UnlockTech` finishes the wiring in one call; loc complete in 8 languages; **no research icon exists**, 19 orphan ones do; no cross-version layout diff is possible). ⭐ Owner's bar on placement is **evidence, not inference**. |
| `HAZARD_KERNEL_PASS.md` | ⭐ **NOT FIRED — for Codex.** Executes the owner's 09-13 rulings on STATE's kernel: the three-part admission test (harm floor · universality · gate) into `STATE_EVICTION.md`, module-list gate (tools/doccheck.py MODULE SETS + tools/upload_preflight.py)/agent/prompts/perma/RELEASE.md § Release rails/editor/version rail (agent/prompts/perma/RELEASE.md § Release rails)/agent/reports/PARKED_OPTIN_REFERENCES.md § Restore rail out to their gates, EF-056 out on a failed harm test (EF-056 keeps the fact), H-04 moved to Rules in force, and a second phase collapsing seven restated rules + the open-decisions enumeration. Two independently committable phases; §6 handover table is written FOR an adjudicating session, which is a different seat by owner ruling. |
