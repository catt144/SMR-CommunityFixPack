...
| `MIGRATION_HUB_HANDOFF_high.md` | `prompt` | **LIVE, single use, authored 2026-09-24** (owner ask): remaining work after the migration audit, cross-check, hub audit and the two sittings on TheGodUncle's save; resweep and S1/S2/S3/S5 filing done (C114-C117); carries the one-release ruling for the hub set, the reporter reply, the build order and the owed measurements; consumed when its last item lands. ⛔ **Not deletable yet, checked 2026-09-26:** items 1/2/5/6 are spent (the hub set merged at `8cb1727`, outbox rows filed by hubset 99 at `ebbcb3b`), but it is the ONLY home of two owner rulings — the load-order deferral (*"we will decide how and what we implement after the fixes are done"*, its `:90`) and what to do if a sitting refutes a set member (*"it depends on the circumstances and information"*). Both verified unique by grep with a positive control. Items 3, 4, 7 and 8 are also open, and hubset 99 named the load-order decision the natural next item, so item 7 is now unblocked rather than spent. Route those two rulings before deleting |
