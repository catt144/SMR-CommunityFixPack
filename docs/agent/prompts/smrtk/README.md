...
| 08b | `08b_STAMP_SITTING_owner.md` | Claude attending (attended) | ✅ keyboard | ⛔ **GATED:** does not fire until **defect 21 is fixed** (`template_name` guard omits every building, so capture returns nothing) and the owner calls the surface stable. Carries blocks 13–15 verbatim — the three native stamps, class 18 — and 08's close. ⛔ Re-runs nothing from classes 1–17 |
