...
## Job 0 — re-validate the cut — this is a CROSS-VENDOR read, and you may re-cut

This chain and its five facts were authored by a Claude session. You are the first other vendor to read them, before
any code exists; that is the point of seating you here. README § "What is FIXED" draws the line: the invariants
are hard, everything else is a default you may change with a stated reason — the API shape, the file layout, the
payload cut, the page design. Read the README's queue, layout and rules against `EF-095`–`EF-099` and the TestKit as it is on disk. If a link is
...
1. **Predictions first.** `reports/SMRTK_SKELETON_PREDICTIONS.md`: for every step 02 will run, the exact
   ...
   against this file.
2. **`Code/70_SMRTK_Core.lua`** (the shape below is a default — a better API is yours to write, recorded under
   DEPARTURES so 03A's payloads inherit the real one) — `SMRTK = rawget(_G, "SMRTK") or {}` at load; `SMRTK.Log(verb, kv)` (the ONE
...
outbox to 02 and 99 **with your DEPARTURES and SUGGESTIONS sections** (the API as actually built is what 03A's
payloads must inherit — put it in 03A's inbox too); strike your row; `git rm` this file; push the pack repo.
