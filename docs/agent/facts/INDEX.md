...
| EF-051 | Steam Cloud RESTORED deleted savegames at launch, which is why the save-dir close-out gate 'failed' twice. ⭐ The post-untick launch has since run, NOTHING came back, and the whole directory delta reconciles — mechanism appears dead; caveat retirement routed to the corun-pt15 audit. ⚠️ Baselines must be recorded as NAMES, never a count | 2026-08-11 | 2026-08-11 | 116 | [EF-051.md](EF-051.md) |
| EF-052 | `SetGameSpeed` accepts factors far above the UI's 20× ultra, but the EFFECTIVE rate is throughput-bound and NON-MONOTONIC — MEASURED 50× at request 50×, then 21×/33×/21× at 100×/200×/400×. ⇒ ask for more and you get less. The hour-skip hazard was NOT reachable on this machine, because the sim's own ceiling sits far below it. ⚠️ Any speed set from the console is wiped by the HUD: `SetGameSpeedState("play")` is hardcoded to `SetGameSpeed(1)` | 2026-08-11 | 2026-08-11 | 64 | [EF-052.md](EF-052.md) |
