...
4. Only then are P4/P6 meaningful — on a stable colony where the drain is not
   ...
   residents" on so landings could not refill the dome. **It would not have
   worked, and it would have invalidated the leg.** Three reasons, in order of
   severity:
   * ⛔ **It does not block landings at all.** `RocketBase:Disembark` →
     `GetDomesReachableByColonists` → `is_welcoming_community`
     (`_GameUtils.lua:342-344`) reads the RAW FIELD `community.accept_colonists`
     and never calls `CanAcceptNewColonists` — which is the METHOD D03 wraps
     (`Opt_ResidencyControl.lua:88-93`). The landing path was the whole reason
     the instruction existed.
   * ⛔ **On the path it DOES block — resettlement, `Colonist.lua:2657` — it
     would mask the guard under test.** D12's symmetric veto exists to stop
     exactly that inflow; with D03 on we would be measuring D03's veto and
     learning nothing about D12's.
   * ⛔ **It is not a shape a player would ever be told to use** (owner,
     2026-08-03): on the resettlement path it blocks **Seniors**, i.e. the
     cohort delivery HARD CONSTRAINT 1 exists to protect.
