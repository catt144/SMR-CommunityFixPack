...
⭐ **The free-work door did NOT suppress the drain, and the reason matters.**
`freework 1` on Sacagawea, yet 8 still movable — because `GetFreeWorkplacesAround`
is colonist-INDEPENDENT (all the arrival seam can use) while the push side asks
`HasFreeWorkplacesAround(colonist)`, which adds `CanWorkHere`. That one slot was
a workplace none of them could take, so the fine test governed and the coarse
number was not the behaviour. ⛔ **Do not read `freework > 0` as "the door should
be holding them."** The N-held-by-1-slot cost stays UNMEASURED — the door never
actually fired, so **P14 is still owed**.
...
**Still owed after tonight:** the split counter read **through a rocket landing**
(the only thing that tests the `ChooseDome` half — a single at-rest reading does
not); P4/P6 completion (does the dome clear, does `overpop` clear — mind the
employed caveat); **P14** (never fired); P13; P12.

⚠️ **One `[LUA ERROR]` in the log, and it is NOT pack code:**
`_GameUtils.lua:446: attempt to index a nil value (local 'dome')` — the operator
snapshot line calling `GetFreeWorkplacesAround(SelectedObj)` with nothing
selected. The module's own call site guards `type(community) ~= "table"` and
checks `labels.Workplace` before calling out. Recorded explicitly so a later log
review does not score it against P11.
