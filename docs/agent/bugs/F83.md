...
⛔ This module became an applies-today HARM, which is why it is removed rather than left inert. The vanilla grant is gone -- `OnMsg.SpawnedAsteroid` now only shows the first-asteroid popup (`Asteroids.lua:418-423`) and hands out nothing -- while our sweep still grants three free Micro-G prefabs and orphans a persisted thread. ⚠️ NO CLEANUP IS OWED AND NONE WAS BUILT: prefabs already granted on a 1.1.0 save cannot be taken back, and the `SMRFixPack_FirstAsteroidPrefabs` GameVar is absent-tolerant, so the save loads clean with the module gone. It goes in the patch notes instead.

⇒ **Fix_FirstAsteroidPrefabs** — removed from the pack in hotfix 2 (re-verification row F-4;
