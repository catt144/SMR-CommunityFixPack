...
The scheduler gates and counts in one expression -- `SessionRandom:Random(100) < descr.spawn_chance and SessionRandom:Random(descr.count_min, descr.count_max) or 0` (`DustDevils.lua:235-237`, under the repeat at `:256`). Our wrapper re-rolls the same gate and burns extra `SessionRandom` draws per tick, which is a divergence rather than a fix.

⇒ **Fix_DustDevilSpawnGate** — removed from the pack in hotfix 2 (re-verification row R-21;
