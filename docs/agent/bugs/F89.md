...
copies: []
---
### F89 — MeteorsDisaster's unbounded drain loop wedges the Meteors thread on ordinary strikes; the colony silently loses all regular meteors (P2, MEASURED)  `[closed: owner ruling 2026-09-21 — FIXED IN VANILLA 1.1.0, source traced. The drain loop's validation changed from table.validate(spawned) at 1.0.7.396349 Meteors.lua:240 to table.validate(spawned, function(descr) return IsValid(descr.meteor) end) at 1.1.0.403908 Meteors.lua:269. spawned holds descriptors, which never go invalid, so 1.0.7 never emptied the list; 1.1.0 checks the meteor inside each one, so a landed meteor clears its entry and the loop exits. Previously: open — filed 2026-08-01 mid-sitting (Tier-1 leg 1); vanilla defect, F78's class on the singles path; COVERED by the F02 watchdog (detect + restart at its 189h threshold, ~6-8 sol latency, proven live the same sitting); no direct fix routable: the loop is mid-function with no seam, and a body copy is barred by F86/FIX_POLICY §3a]`
