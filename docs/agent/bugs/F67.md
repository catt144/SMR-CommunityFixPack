...
#### 2026-09-12 - still-needed sweep: Current minimum-wait topology

On **1.1.0.403908**, shared IsCargoReady imposes a minimum one-hour automatic wait on both sides (`Lua/UniversalRocket.lua:554`), superseding the older only-asteroid brake explanation. Empty requests still report ready (:558) inside a longer configured wait window. The wrapper retains the flight-policy/forced timer exits and targets that remaining premature-empty departure. No current dwell, launch or fuel outcome was newly witnessed.

Current review: [one-module evidence](../reports/still-needed/Fix_LanderEmptyLaunch.md). Historical dates and status below remain unchanged. Owner decisions: checklist 156.
