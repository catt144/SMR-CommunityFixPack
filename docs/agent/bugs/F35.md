...
`SmartHome_Small` examples, so *"an applied upgrade exists"* is measured and
*"the Hospital's Remote Medic is among them"* is not. And **PT-35 leg A's
turbine half remains UNRUN** — no sanitizer pass executed in this run, because
the pack was not loaded. The chain's stop condition 3 (a broken fixture) is
**not** triggered: the fixture is fine; the pack was off.
