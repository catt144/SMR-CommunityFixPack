...
seq: 62
row: 62
title: "Services use neighbouring domes, including shared Passage Hub spokes; never trains"
status: "wontfix"
status_source: "tag"
priority: "P2"
evidence: "high"
row_status: "wontfix retained; 2026-09-11 adjacency rederived, shared Passage Hub spokes are neighbours; one physical hop wording corrected; desk topology controls"
updated: "2026-09-11"
copies: []
---
...
## 2026-09-11 — adjacency remains, but Passage Hubs qualify the old wording

**SOURCE:** `Dome:GetClusterDomes` returns `connected_domes` (`Dome.lua:745-747`),
not the transitive `dome_network` built at `:688-706`. Resident service helpers
and selection consume that adjacency (`:828-830`, `:3501-3519`). Ordinary A–B–C
dome chains still omit C from A's service search. But `ConnectDomeToHub`
(`Passage.lua:1592-1600`) adds every attached dome pair via `ConnectDomePair`
(`:1572-1589`), so services ARE available between shared Passage Hub spokes.
"Exactly one physical passage hop" is too broad on 1.1.0.

**DESK-CONTROLLED:** real adjacency/network builders and selector with eligible
service fixture in `tools/desk_migration_observations.py`; ordinary chain omits C,
shared hub includes C, passage-services off excludes it. No game run. The old
walkability-versus-service observation remains for ordinary chains; no design
change proposed. Original-game provenance was not rederived. Report §10; ck151.
