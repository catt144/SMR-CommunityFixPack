...
> takes over from `ModsReloaded` — colony, full suite, quit. The full six-step
> recipe is at the top of `Code/98_EnablePathLeg.lua`. **One click stays manual
...
>   is disarmed.
>
> 🔧 **TWO PROCEDURE FACTS LEARNED BY RUNNING IT** (carried here and enforced by the leg):
