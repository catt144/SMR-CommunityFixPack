...
priority: "P2"
evidence: "med"
row_status: "investigating; 2026-09-11: 1.1.0 loop-seam per-track omission desk-controlled with real route builder; buildability and July incident causation unproved; old union-list falsifier corrected"
updated: "2026-09-11"
copies: []
---
...
## 2026-09-11 — loop-boundary omission controlled; historical cause still unproved

**SOURCE:** live `ForEachStationAlongTrack` (`TrainTransport.lua:371-455`) retains
first-index subtraction (`:373-378`), endpoint wrap and raw-stride increment
(`:397-409`, `:453`). New `trfRunnableOnly` checks (`:384`, `:425-427`) do not
repair that enumeration. Route construction (`:251-300`) remains structurally
the same as archived 1.0.7. `Train:TransferCargo` reads one departure track
(`Train.lua:836-841`, `:887`); destination matching precedes both boarding and
the capacity penalty (`:962-979`).

**DESK-CONTROLLED, `tools/desk_migration_observations.py`:** supplied three-station
paired connectors A–B–C–A, with real `EnumRouteTracks`, yield `[A,B,C]`, loop true,
three edges. C→A gets stride -2, visits A and then already-visited C, and omits B.
A→B visits B,C; C→B visits B,A. Open-line and cargo-only controls discriminate.
This needs neither a duplicate station nor a missing edge. Therefore the older
"any stride other than ±1 hard-returns on a missing edge" claim is too strong.
No full boarding simulation, engine geometry or physical buildability check ran.

**Correction to the older diagnostic:** `Station:GetReachableStations`
(`Station.lua:426-433`) merges enumerations across connected tracks. A working
opposite direction can mask the failing departure's omission. Both endpoints
listing each other does **not** falsify the per-track hypothesis. The settling
capture must identify the actual train's departure track and enumerate that
track. If it includes the Waiting ticket's destination, investigate downstream
boarding/eligibility instead. A one-way hole remains useful evidence, not a
required signature of every omission.

**INFERRED next step:** provision the loop recipe in report §13 and capture before
mitigating a recurrence. Do not implement raw-stride normalization: even the sign
is wrong at this seam, and duplicate occurrences require an edge/occurrence-aware
solution. `stations_visited` remains shared (`:369`, `:391-394`); diagnostic
callbacks must not enumerate recursively.

Historical observed waiting remains **UNVERIFIED ON 1.1.0** and unexplained.
The 2026-07-28 witness is 45 days old; no verbatim runtime log was recovered here.
Current desk evidence is a sharper candidate, not proof of that incident's cause.
Status stays `investigating`; no fix built. Owner handoff: ck151, report §13.
