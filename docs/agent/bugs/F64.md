...
**Retired from the 1.1.1 pack:** `Fix_TrainsToVoid` and its load/editor registrations and TestKit probe requirements are removed. The status above records the historical fix, not a newly attended 1.1.1 result.

**SOURCE, archived 1.1.1.405907:** Lua/Buildings/Station.lua:289-299 calls train:DestroySilent("station", bld) directly when the station is demolished.
...
#### 2026-09-12 - still-needed sweep: Current storage implementation scope

On **1.1.0.403908**, the pre-demolition hook uses DestroySilent -> DoDemolish -> OnDemolish to return the Train prefab before the Station event bare-deletes current_station matches (`Lua/Buildings/Station.lua:292-293`; `Lua/Units/Train.lua:133`, :190). The counter still reaches send-out UI and valid-track assignment. Train.Done guards and historical compensation described in older proposals are absent from the installed module. Redeployment requires an eligible surviving or rebuilt route; vanilla can also construct replacement trains. No current deletion/refund/redeployment cure was newly tested.

Current review: [one-module evidence](../reports/still-needed/Fix_TrainsToVoid.md). Historical dates and status below remain unchanged. Owner decisions: checklist 156.
