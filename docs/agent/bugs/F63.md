...
priority: "P2"
evidence: "high"
row_status: "wontfix retained; 2026-09-11 source recheck: training still absent from emigration score; train-permitted vs automatically offered discrepancy remains, old F61 gating changed"
updated: "2026-09-11"
copies: []
---
...
## 2026-09-11 — scoring omission remains a feature decision, not a broken hookup

**SOURCE-READ ONLY**, 1.1.0: migration (`Colonist.lua:3425-3495`) and workplace
evaluation (`Workforce.lua:83-128`; `Dome.lua:3521-3535`) do not score training
slots. `ChooseTraining` (`Dome.lua:3537-3545`) uses resident work domes and
`labels.TrainingBuilding`. The permitted-versus-offered discrepancy remains:
`ShiftsBuilding.lua:302-320` accepts an out-of-cluster transport route, while
automatic dome training selection does not enumerate stations. Manual training
reachability is at `TrainingBuilding.lua:147-164`.

The old "F61-gated" selection wording does not carry forward: those explicit
quarantine checks were removed in the rewrite (F61 recheck). No original-game
provenance rederived, no fresh game observation, no reinstatement. Report §11
has the expensive eligible-student/manual-vs-automatic diagnostic and controls.
