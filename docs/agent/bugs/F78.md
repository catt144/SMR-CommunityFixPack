...
## ⭐⭐ THE DEFECT OCCURRED IN A LIVE CAMPAIGN AND THE FIX HEALED IT — 2026-08-03 (log `Mars.exe-20260803-13.30.54-6a22b86d`)

Not a probe, not a fixture: an **organic** storm wedge in the owner's own save,
caught free during the PT-62 D12 sitting.

```
762  MeteorStormWedge: WEDGE confirmed - g_MeteorStorm set for 1h+ with no notification and nothing falling (scheduler thread alive but stuck); healing
763  MeteorStormWedge: scheduler thread restarted - future storms will schedule again
765  MeteorStormWedge: forced storm state clean (14 stray meteor object(s) removed)
```

Every element of this entry's diagnosis, observed live: `g_MeteorStorm` latched
with the scheduler thread **alive but stuck**, no notification, nothing falling —
and **14 stray meteor objects** left behind, which the heal removed.

⭐ **The owner's own impression corroborates it independently and from outside
the log:** *"I think it's been a good long time since this map had a storm."*
That is exactly what a wedged scheduler produces — `future storms will schedule
again` says storms had been blocked, and the player had noticed without knowing
why. Symptom and mechanism met.

**Sequence in the same session, three fixes in a row, each on its own defect:**
`F02`'s watchdog restarted a Meteors thread stuck for 198 game hours (:721) →
`F81`'s rains guard caught a rain colliding with the now-active/predicted
disaster (:725) → this wedge confirmed and healed (:762-765) → meteors resolving
normally again, 3 game hours since the last (:767). ⚠️ The ORDER is logged; the
CAUSAL chain between them is inference and is not claimed as measured.

⭐ **SECOND ORGANIC FIRING - 2026-08-05** (corun-batch-1 sitting, log
`docs/archive/cb1sitting_Mars.exe-20260805-14.28.49.log`, uptime `1:01:55`):
`WEDGE confirmed - g_MeteorStorm set for 1h+ with no notification and nothing
falling (scheduler thread alive but stuck); healing` -> `scheduler thread
restarted` -> `forced storm state clean (11 stray meteor object(s) removed)`.
Same shape as 2026-08-03, on a staged copy of the same save, unprompted.
Recorded by the terminal audit - the sitting's own review missed it.

⭐⭐ **THIRD AND FOURTH ORGANIC FIRINGS — 2026-08-11, BOTH IN ONE SITTING**
(`corun-pt15` attended sitting, log
`docs/archive/cp15sitting_Mars.exe-20260811-15.09.30.log`). Staged copy
`CP15STAGE.savegame.sav` of the owner's `PT-15` fixture — a **different colony
and a different map** (`BlankBigTerraceCMix_10`) from the 08-03/08-05 pair, so
this is no longer the same save wedging repeatedly.

| # | uptime | log lines | strays cleared |
|---|---|---|---|
| 3 | `2:00:53` | L973-L977 | **2** |
| 4 | `2:28:25` | L1151-L1154 | **3** |

Both are the identical three-line shape: `WEDGE confirmed - g_MeteorStorm set
for 1h+ with no notification and nothing falling (scheduler thread alive but
stuck); healing` → `scheduler thread restarted - future storms will schedule
again` → `forced storm state clean (N stray meteor object(s) removed)`.

**Unprompted and unforced.** Nothing in the sitting touched meteors, storms or
disasters; the owner was playing their own colony through the St. Elmo's Fire
mystery. ⇒ four organic firings now stand across **two different saves and two
maps**.

⚠️ **Confound stated rather than buried: the march ran at a MEASURED 50x time
factor** (see the sitting's speed measurement), well above the 20x UI ultra.
Whether a high time factor makes the underlying wedge *more likely* is **not
established** — it is equally consistent with 50x simply compressing more game
hours into the sitting and so surfacing a rate that was always there. Two
firings in ~28 minutes of real time is not a rate that can be compared with the
earlier singletons without controlling for game hours elapsed, and this sitting
did not control for it. Recorded as an open question, not as evidence either way.
...
The wedge route is closed upstream. The drain loop validates on `IsValid(descr.meteor)` (`Meteors.lua:267-270`, `:273`), the scheduler is a `MapGameTimeRepeat` (`:297`), and a savegame fixup kills any thread left inside `MeteorsDisaster` and ends a stuck storm (`:390-412`). ⚠️ Residual named rather than hidden: the loop is still unbounded if a meteor object stays valid without ever posting `MeteorDone`. Nothing measured that; a module for it would be NEW code against the new mechanism, which is out of this chain's fence.
