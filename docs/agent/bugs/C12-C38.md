...
- **C25 [mechanism verified, trigger unproven] — Jumbo Cave reinforcements
  ...
  **1.0.7.396501**, NEWER than our pinned 396349 that all of the above Src is
  ...
  attachment). ⇒ still exactly one thing missing: the rider's console read
  (`WasteRockObstructor` count in the underground drones' `unreachable_buildings`)
  taken **while a site is stuck** — obtainable now off the owner's own live repro,
  no reporter save required. Non-zero ⇒ C25 earns its F-row; zero ⇒ the wedge is
  something else.
