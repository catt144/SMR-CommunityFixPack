...
⭐ **THIRD ORGANIC FIRING — 2026-08-11** (`corun-pt15` attended sitting, log
`docs/archive/cp15sitting_Mars.exe-20260811-15.09.30.log`, uptime `2:42:44`,
L1231): `MeteorFrequency: WATCHDOG — no meteor resolved for 211 game hours
(thread ALIVE but stuck); restarting onto vanilla's body`. A **different save and
a different map** from the 08-03/08-05 pair (`CP15STAGE`, a copy of the owner's
`PT-15` fixture, `BlankBigTerraceCMix_10`), unprompted — nothing in the sitting
touched meteors. The same load also fired the one-shot latched heal at L230
(`persisted Meteors thread was alive; restarting onto vanilla's body,
latch false -> 1.0.1`), which is the ordinary load-time path, not a wedge.
⚠️ **Confound, stated rather than buried:** the march ran at a MEASURED 50x time
factor (`agent/facts/EF-052`). Whether a high time factor makes the stall more
likely is **not established** — it is equally consistent with 50x simply
compressing more game hours into one sitting. Not controlled for here.
...
The dead `if` went with the thread it lived in. 1.1.0 runs meteors from a `MapGameTimeRepeat` that stores `g_NextMeteorsTime` as a `GameVar` (`Meteors.lua:293-322`, and the repeat at `:297`), and `_G.Meteors` is now `GameVar("Meteors", false)` kept only for a savegame fixup. Our thread-keyed wrapper could never match it, and the watchdog's `RestartGlobalGameTimeThread("Meteors")` would call `CreateGameTimeThread(nil)`.

⇒ **Fix_MeteorFrequency** — removed from the pack in hotfix 2 (re-verification row R-9;
