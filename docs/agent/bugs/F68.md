...
The lander cargo path was rewritten: the loaded hold is added back and the budget is allocated by fair share, a different shape from our ratchet (`UniversalRocket.lua:2046`, `:2069-2086`, `:2537-2546`). ⛔ Independently, the module could no longer run at all -- see F113: it calls `GetEarthExportResPossibleReward`, which has **0 hits** in the 1.1.0 tree.
