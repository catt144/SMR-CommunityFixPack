...
The vanilla data is now correct, and our module's own 'already correct' latch IS the evidence of it: every storage-like condition in `Data/FactionDef/LastTransmission.lua` sets a `ScriptCheckDomesGridStorage` with the right `Resource` (12 such entries; e.g. `Resource = "Oxygen"` at `:245`). ⚠️ Worth carrying forward as a general signal: a `DataPatch` that logs 'already correct' is a RETIRE candidate, not a healthy one.

⇒ **Fix_LastTransmissionStorage** — removed from the pack in hotfix 2 (re-verification row R-13;
