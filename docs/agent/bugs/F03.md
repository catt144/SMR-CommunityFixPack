...
Fixed upstream at BOTH ends. The leak source: `Building:StopUpgradeModifiers` now iterates with `pairs`, not `ipairs` (`Building.lua:1303-1311`), so it actually turns the modifiers off and our second `TurnOff` is idempotent and useless. The residue already in old saves: 1.1.0 ships `SavegameFixups.RemoveLeakedUpgradeModifiers` (`Building.lua:1313-1345`), which strips leaked `<handle>_upgrade<tier>_mod_<i>` entries keyed on live ownership -- a stricter test than our handle-resolution one. ⇒ `Fix_UpgradeModifierLeak.lua` was deleted AND the matching F03 pass was removed from `90_SaveSanitizer.lua`. ⚠️ FILED, NOT FIXED, and it does not change this verdict: vanilla's new fixup ends `for _, entry in ipairs(leaked) do` with `leaked` left `nil` when a container has no leaks, unguarded, where vanilla guards `ipairs(x or empty_table)` in 36 other places under `Lua/`. If `ipairs(nil)` raises here it aborts the sweep at the first clean container while `procall` marks the fixup applied anyway. UNPROVEN -- it needs a run, not a read.
