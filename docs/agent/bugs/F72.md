...
Gate and list are now built from the same two helpers, so they cannot disagree: `IsRocketAvailableForFlight` plus `GetAvailableFlightLocations()` (`PlanetaryView.lua:245-252`, `:258-265`; `PlanetUI.lua:1701-1710`).
