...
| 128 | C48 | Wild vegetation offers Seeds to the drone network with NO distance penalty, while the only comparable scattered source in the game (SurfaceDeposit) carries a deliberate 50% one — so on a vegetated map hundreds of one-plant trickle offers compete with a full depot on equal terms | filed | ? | SOURCE-VERIFIED (mechanism) + owner-observed (drone behaviour, attended sitting) — ⛔ the PREFERENCE is NOT measured | [C48.md](C48.md) |
