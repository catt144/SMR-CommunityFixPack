...
| 132 | C52 | The in-game Mod Manager can never show a mod's screenshots — the download loop reads `mod_prefix` outside the block that declares it, so it concatenates nil on the first screenshot — and its thumbnail cache is keyed on mod id + version, so a replaced preview image is never re-fetched | filed | ? | SOURCE-VERIFIED (the out-of-scope local, `CommonLua/Libs/Paradox/ParadoxMods.lua:213-276`, re-derived by symbol; the version-keyed cache at `:221-225`; the developers' own `-- todo: this is not working` at `:247` and `CommonLua/UI/ModManager.lua:786`) + ⭐ THEIR STATED MECHANISM IS REFUTED BY OUR OWN `EF-008` (the `ScreenshotUrls` assert does not halt, and `ProtectedPropertyObject.__newindex` rawsets after asserting, so the assignment SUCCEEDS in retail and line 799 IS reached) — ⛔ not observed by us in the Mod Manager, ⛔ no fix proposed, ⛔ not a release gate. ⚠️ RELEASE-RELEVANT: our own three mods are about to be uploaded with preview art | [C52.md](C52.md) |
