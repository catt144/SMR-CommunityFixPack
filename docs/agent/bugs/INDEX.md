...
# Bug index — 161 rows, 126 entry files

103 F + 12 D + 46 C. `seq` is the entry's position in the old BUGS.md file order;
...
| 90 | F85 | Breakthrough choice popups + Assembly "Colony Values" choice ride real-time waiters — a save in their open window voids the choice | filed | P3 | latent | [F85.md](F85.md) |
...
| 114 | C39 | `Policy_Automation_ServiceAutomation` cuts `max_workers` by **label** while its performance compensation keys on **class** — the four Workshops are on the label and outside the class | filed | ? | **OBSERVED** | [C39.md](C39.md) |
