...
# Bug index — 159 rows, 124 entry files

101 F + 12 D + 46 C. `seq` is the entry's position in the old BUGS.md file order;
...
| 124 | C46 | St. Elmo's Fire — the `"destroy"` branch kills every wisp but never clears `el_prod_modifier`, so an emptied Light Trap keeps producing its old power FOREVER | filed | ? | **OBSERVED** | [C46.md](C46.md) |
