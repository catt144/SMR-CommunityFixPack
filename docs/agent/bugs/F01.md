...
The game rule is now honoured by the scheduler itself: the underground marsquake repeat's condition ends `and not IsGameRuleActive("NoDisasters")` (`Marsquake.lua:353`, and the same guard at `:67`). Our slot-3 wrapper is redundant.

⇒ **Fix_CaveInsNoDisasters** — removed from the pack in hotfix 2 (re-verification row R-18;
