...
**Retired from the 1.1.1 pack:** `Fix_FounderTraitNotification` and its load/editor registrations and TestKit probe requirements are removed. The status above records the historical fix, not a newly attended 1.1.1 result.

**SOURCE, archived 1.1.1.405907:** The vendor removed both the feature handler and its FounderGainsTrait preset. CommonLua/Libs/Notifications/Notifications.lua:25-48 falls back to the base notification class for an unknown id; deletion prevents that call. This is feature removal, not a vendor repair of F23.
