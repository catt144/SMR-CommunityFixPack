...
The train cap is now derived from the route's station count: `TrackBase:CanAddVehicle` does `local trains, cap = GetTrainsOnRoute(self)` and returns `trains < cap` (`Track.lua:423-426`), so `max_vehicles` is display-only. ⛔ CONSEQUENCE TO CARRY, not a silent drop: removal leaves **two displays stale** -- the x/max train number stops refreshing after a salvage. Cosmetic, and it belongs in the patch notes rather than being dropped quietly (QA section 0.8).

⇒ **Fix_TrainMinors** — removed from the pack in hotfix 2 (re-verification row R-34;
