---
id: "F94"
seq: 99
row: 106
title: "An operator-precedence slip makes **any parked supply rocket** satisfy the asteroid-visit gate — the picker opens with no lander to pick"
status: "fixed"
status_source: "tag"
priority: "P3"
evidence: "SOURCE-VERIFIED"
row_status: "**fixed 2026-08-02 (chain prompt 8b) inside the existing `Fix_AsteroidLanderAvailable` — VERIFIED IN PLAY 2026-08-02 (PT-60).** Promoted from C24 and APPROVED 2026-08-02 (chain prompt 7 §4 package, pre-cleared); built as the specced §1.4b body copy (12 lines from `PlanetaryView.lua:433-444`, build 1.0.7.396349, one added pair of brackets) with F72's scan appended where vanilla's `return false` was; no new registry id; probe `AsteroidVisitPrecedence` written. **F72's module header and its BUGS entry were both corrected in the same commit** — the chained delegation they advertised is gone, because a wrapper cannot filter a false positive. `PlanetaryView.lua:439` — `and` binds tighter than `or`, so `IsKindOf(rocket, \"LanderRocketBase\")` qualifies only the first of three clauses. **R1**: `SupplyRocketBase` descends from `RocketBase` alone (`SupplyRocket.lua:1-3`) and parks in `WaitLaunchOrder` (`RocketBase.lua:643, :699`), which the detached clause accepts. The picker's own list keeps only non-pod `UniversalRocketBase` rockets (`PlanetUI.lua:1623-1635`), so it comes up empty. Tells: a guard that cannot fire, and the branch 3 lines above plus `EarthVisitPossible` both bracket the same idiom correctly. ⚠️ Repairing a false positive means owning the predicate — F72's chained delegation cannot survive, and its header says otherwise today (entry)"
updated: "2026-08-02"
copies: []
---
### F94 — An operator-precedence slip makes ANY parked supply rocket satisfy the asteroid-visit gate (P3, SOURCE-VERIFIED)  `[fixed 2026-08-02 by chain prompt 8b inside the existing Code/Fix_AsteroidLanderAvailable.lua — §1.4b body copy of PlanetaryView.lua:433-444 (1.0.7.396349) with the brackets restored, F72's scan appended where vanilla's `return false` was; no new registry id. **F72's header AND its BUGS entry were corrected in the same commit** — the chained delegation both advertised is gone. Probe AsteroidVisitPrecedence written. VERIFIED IN PLAY 2026-08-02 (PT-60)]`

**Defect, on the line (`PlanetaryView.lua:439`).**

```lua
elseif IsKindOf(rocket, "LanderRocketBase") and rocket.command == "Refuel"
        or rocket.command == "WaitLaunchOrder"
        or (rocket.command == "LoadAndLaunch" and not rocket.target_spot) then
```

`and` binds tighter than `or`, so this parses as
`(IsKindOf(…) and cmd=="Refuel") or (cmd=="WaitLaunchOrder") or (cmd=="LoadAndLaunch" and not target_spot)`
— **the class test governs only the first of the three clauses.** Two thirds of
the predicate apply to whatever rocket reached the `elseif`.

**Reachability: R1, and it is not a corner — it is the default state of a
supply rocket.** Enumerated this session: `SupplyRocketBase.__parents =
{ "RocketBase" }` (`SupplyRocket.lua:1-3`) — **not** `UniversalRocketBase`, so
the first branch does not catch it, and **not** `LanderRocketBase`, so the class
test is false. `RocketBase` parks in `WaitLaunchOrder`
(`RocketBase.lua:643, :699`; `SupplyRocket.lua:145` treats it as the landed-idle
state), and every rocket in the loop comes from `MainCity.labels.AllRockets`.
So **one supply rocket sitting on its pad awaiting a launch order makes
`PlanetaryAsteroidVisitPossible()` return true** — with no lander anywhere in
the colony. The same holds for the trade/refugee/foreign-aid/expedition rockets,
all of which descend from `SupplyRocketBase` and use the same command.

**Consequence.** The one consumer is the VISIT ASTEROID action
(`PlanetaryViewAsteroidResources.generated.lua:37-41`): `if
PlanetaryAsteroidVisitPossible() then host:SetMode("rockets") else
PromptNoAvailableAsteroidLanders() end`. The list that mode then builds,
`LandingSiteObject:GetRocketsForExpedition` (`PlanetUI.lua:1623-1635`, the
function F72 already established as the list builder), keeps only
non-supply-pod `UniversalRocketBase` rockets — **so the supply rocket that
opened the screen cannot appear on it.** The player gets an empty picker instead
of the clear "no asteroid landers" prompt the code was reaching for.

**Intent tells — two, and the second is inside the same function.**
1. **Dead validation (§4 tell 2).** `IsKindOf(rocket, "LanderRocketBase")` is a
   guard that cannot do its job for two of the three commands it was written to
   qualify.
2. **⭐ Self-contradiction within one function (§4 tell 4).** The branch three
   lines ABOVE parenthesises its or-group correctly — `not rocket.arrival_loc
   and ((cmd == "CmdWaitOrder" and …) or (cmd == "CmdOnEarth" and …))` (:436) —
   and so does the sibling function immediately below,
   `EarthVisitPossible` (:446-453). The same author, in the same file, in the
   same idiom, got it right twice and dropped one pair of brackets once.

**§4a — who benefits: the player.** A misleading UI state in vanilla, reachable
by anyone with a supply rocket. No mod involved.

**Complementary to F72, confirmed by reading our own module.**
`Fix_AsteroidLanderAvailable` is a **post-wrapper** whose first line is
`if orig(...) then return true end` — so it repairs the false NEGATIVE and
**passes vanilla's false positive straight through**. C24 survives our pack
today exactly as the candidate said.

## Fix spec (approved; build = chain prompt 8b — prompt 8 split under rule 3)

**Land it inside `Code/Fix_AsteroidLanderAvailable.lua` — one module, one
target, both halves.** No new registry id.

**Technique: §1.4b global replacement with a body copy** — 12 lines from
`Lua\PlanetaryView.lua:433-444` (game **1.0.7.396349**), byte-identical except
the parentheses:
`elseif IsKindOf(rocket, "LanderRocketBase") and (rocket.command == "Refuel" or
rocket.command == "WaitLaunchOrder" or (rocket.command == "LoadAndLaunch" and
not rocket.target_spot)) then`. F72's permissive scan then runs on top of the
corrected body, in the same order it runs today.

⚠️ **Stated cost, because it reverses a property that module's header currently
advertises: the chain is lost.** F72 deliberately delegates to `orig` so any
other mod's wrapper survives. A false POSITIVE cannot be filtered by a
wrapper — the wrapper sees only `true`, never which rocket produced it — so
repairing C24 means owning the predicate. Whoever builds this **must update the
F72 header**, which currently explains the delegation as a design choice.

**§3a: nothing enters the save.** The function is synchronous and is called only
from an `XAction` handler — UI, real-time, and UI windows are explicitly safe.

**Probe outline:** `AsteroidVisitPrecedence` — with the fix active, a scan
asserting that `PlanetaryAsteroidVisitPossible()` is false in a colony whose
`AllRockets` label contains only non-`LanderRocketBase`, non-`UniversalRocketBase`
rockets in `WaitLaunchOrder`. Cheap to state; the live half rides the F72
checklist item.

**Intent statement for the header:** *the class test was meant to qualify all
three commands — the branch above it and the function below it both write the
same idiom with the brackets in place. We add the brackets. No rocket that
legitimately qualifies today stops qualifying.*

Cross-refs: **C24** (the candidate this promotes), **F72** (same function,
opposite error, same module), `PlanetUI.lua:1623-1635` (the list that disagrees).

## ✅ BUILT 2026-08-02 (chain prompt 8b) — inside `Code/Fix_AsteroidLanderAvailable.lua`

The twelve lines were re-read at `Lua\PlanetaryView.lua:433-444` this session and
copied byte-identical except the one added pair of parentheses, marked
`-- FIX (F94)`. F72's permissive scan now sits where vanilla's `return false` was
— the same position, in the same order it ran as a post-wrapper — so F72's repair
is unchanged in behaviour. One module, one target, one registry id, both halves.

### ⚠️ The advertised property this removes, and where it is now corrected

**Both** places that described the old shape have been fixed in the same commit,
because leaving either would have the module claiming behaviour it no longer has:

* the **module header**, which explained the delegation as a design choice
  ("purely permissive … leaves the shipped logic, and any other mod's wrapper,
  completely intact");
* the **F72 BUGS entry**, which said the same thing in the same words and also
  carried it in its heading tag.

The reason the chain cannot survive is worth stating once more, since it is the
general rule and not a fact about this function: **a wrapper cannot filter a false
positive.** It sees only the boolean the shipped body returned and never learns
which rocket produced it, so there is nothing to filter on. Repairing a false
positive always means owning the predicate.

⭐ **The same shape came up in prompt 8's `Fix_ShuttleHubOffAvailable` conversion,
and the honesty discipline carries over**: record what the technique does NOT buy
in the same breath as what it does. Here that is — the module can no longer chain
onto another mod's wrapper, and if a future patch changes this predicate we
reinstate the 1.0.7 shape rather than inheriting theirs. The apply-time self-check
catches a renamed or removed target, **not** an edited same-named body; the
after-every-patch extraction diff is what covers that (WORKFLOW.md).
(The mirror-image note: unlike ShuttleHub, nothing here is *duplicated* — the
copied body replaces vanilla's rather than re-running beside it.)

**One redundancy kept deliberately.** F72's scan opens with defensive
`rawget(_G, "MainCity")` / `MarsScreenLandingSpots` reads that mattered when it
ran before vanilla's body could raise. The copied body now dereferences both
directly and first, so those reads can no longer be the thing that saves us. They
are **noted in the header and left in place** — this is `tested` code and the
redundancy costs nothing; removing it would be an untested change to a passing
module for tidiness.

**Probe:** `AsteroidVisitPrecedence` — asserts `PlanetaryAsteroidVisitPossible()`
is false for a colony whose `AllRockets` holds only non-`LanderRocketBase`,
non-`UniversalRocketBase` rockets in `WaitLaunchOrder`. The live half rides the
F72 checklist item. ⚠️ **The existing F72 probe was checked against the new body
and is unaffected**: it stubs `IsKindOf` to answer true only for
`"UniversalRocketBase"`, so all five of its cases take the first branch and never
reach the corrected `elseif`.

---

## 2026-08-12 — P5 no-regression, re-derived rather than inherited (`corun-pt60`)

PT-60's **P5** named this as one of two probes at risk from the batch. Both halves checked:

* **Re-derived from source 2026-08-12** (not trusted from the note above):
  `AsteroidLanderAvailable` in `30_Probes_Wave3.lua` stubs `IsKindOf` to answer true only for
  `"UniversalRocketBase"`, so **all five cases take the FIRST branch and never reach the
  corrected `elseif`** — the probe genuinely cannot be disturbed by F94's rewrite.
* **Ran and PASSED with the message byte-identical to the 2026-08-11 retail baseline:**
  `PASS AsteroidLanderAvailable [behavior] busy-but-unassigned landers on the pad count;
  assigned and off-world ones do not`
  (`docs/archive/cp60sitting_Mars.exe-20260812-13.38.15.log:739`).
* Module `AsteroidLanderAvailable` read `active` with an empty detail (it is one of the two
  **amended-in-place** ids, so its apply verdict must not move — it did not).

⇒ No regression. ⚠️ The **live** half (a supply rocket parked while the picker is opened) is
still owed and rides the F72 checklist item — this leg did not sample it. `fixed` unchanged.

## ⭐ 2026-09-08 — game 1.1.0: what changed, and what hotfix 2 did about it

⛔ **Nothing above this line is edited.** The 1.0.7 record stands exactly as it was
written and every citation in it was true of **1.0.7.396349**. This section adds only
what game **1.1.0.403908** changed, read against the shipped 1.1.0 body on 2026-09-08.

Same unification as F72 -- one availability helper feeds both the gate and the list (`PlanetaryView.lua:245-252`, `:258-265`).

⇒ **Fix_AsteroidLanderAvailable** — removed from the pack in hotfix 2 (re-verification row R-11;
owner decision 98, ruled 2026-09-08: delete rather than gate, because the live pack serves the
current patch period and 1.0.7 players are served a frozen v5 instead, decision 118).

⛔ **What may NOT be read into this.** Not that the fix was never needed — it was
correct on 1.0.7, and what is true is that 1.1.0 fixes it itself. Not that removal is
verified safe — it is verified **in source**; nothing has been run in a game. Not that no
player is affected — a player still on 1.0.7 who takes this update loses this fix.
No status word is moved here, because no boot log has been taken since the change.
