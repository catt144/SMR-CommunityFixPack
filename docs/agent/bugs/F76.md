...
message log; the since-dissolved `PLAYTEST_HELP.md` already recorded "prefer a
bare expression for a simple read", and this is a second instance of its PT-61
*silence reads as a dead console* lesson):
...
**closed — REFUTED 2026-08-03 (chain-12 QA, job 10):** the named mechanism is falsified by measurement + source (box and mouse are one coordinate space; the original forensic box was correct placement), the load-failure claim did not reproduce, and the reporter's premise (that the picker was ours) is corrected. Nothing built, nothing shipped. **The OG witness (a DIFFERENT symptom — the icon does not appear) and the M5 out-of-range-mouse lead are NOT closed with it — filed as `C41`** (entry)
