...
BOTH halves of this entry lost their module, for different reasons.

**(b), the deadlock -- `Fix_RainsDeadlock`, row R-14.** Rains now run as a repeat cycle with an explicit condition, not an untimed `WaitMsg`: `RainsDisasterRepeatCycle` (`TerraformingDisasters.lua:363`), `RainsDisasterRepeatCondition` (`:389`) and `GameTimeRepeat("RainsDisaster_toxic", ...)` (`:396`). ⛔ Re-arming would now be HARMFUL: our wrapper's `Msg("RainDisasterEnd")` drives a lightmodel handler in 1.1.0, and the migration `SavegameFixups.RainsDisasterLoopToRepeat2` (`:483`) is the only remaining mention of the old loop. The C34 heal goes with it.

**(a), the stranded prediction flag -- `Fix_DisasterPredictionLeak`, row R-20.** Vanilla fixed the leak: `EndMeteorStorm` removes the notification first (`Meteors.lua:1204-1212`) and `OnMsg.RemoveNotification` clears the flag on expiry. ⛔ But the module did not just become inert -- the QA promoted it to an **applies-today harm**, and re-derivation confirms it. 1.1.0 sets `g_DisastersPredicted.DisasterNormalRains = true` during a normal-rain warning (`TerraformingDisasters.lua:349`) with **no notification behind it**, while our `NewDay` sweep clears every flag that has no live notification (`Fix_DisasterPredictionLeak.lua:97-103`). On the day the sweep lands in that window it cancels the game's own prediction gate, and a dust storm or cold wave can start on top of an incoming rain warning. Removal IS the fix.

⇒ **Fix_RainsDeadlock + Fix_DisasterPredictionLeak** — removed from the pack in hotfix 2 (re-verification row R-14 / R-20;
