...
## 2026-09-11 — current selector rechecked; owner decline unchanged

**SOURCE-READ ONLY**, 1.1.0: `Dome:GetService` (`Dome.lua:3501-3519`) consumes
resident service domes (`:828-830`), without station enumeration. Workplace
selection does enumerate station routes (`:762-798`, `:812-821`). F62 qualifies
the physical passage-hop wording for new Passage Hubs. No new in-play witness,
and no feature proposed or parked. The historical F80 symptom remains unexplained;
its new desk per-track omission is not a causal explanation of that incident.
`reports/MIGRATION_DEV_REPORT.md` §12–13 records the limits.
