...
⭐ **Re-verified on the rig 2026-08-04 (unattended-1 leg D2), from the defect
state rather than from a healthy save.** Run conditions: retail `Mars.exe`
**1.0.7.396349**, staged COPY (`U1STAGE.savegame.sav`) of `TEST2H TRAIN`, pack
**81/81 active as READ**, 4 loads / 2 throwaway saves. Log:
`docs/archive/u1c6_Mars.exe-20260804-17.24.57.log`. FORCED: one assignment,
`SMRFixPack_MeteorLatch = false`, recreating the pre-heal state. ORGANIC:
nothing. Ceiling: **MECHANISM** — this says how the heal behaves when it fires,
not how often a real save needs it.

The post-force read confirms the write landed (`D2 latch now=false`), so the
heal had real work to do. On the next load the pack printed **exactly once**:

> `MeteorFrequency: one-shot heal — persisted Meteors thread was alive;
> restarting onto vanilla's body (latch false -> 1.0.1)`

and the latch read back `1.0.1` — **exactly the baseline value, never above it**.
A further save + reload of the healed file printed **no heal line at all** and
read `1.0.1` again. So all three of the leg's conditions hold: the line appears
after the healing load, it is **absent** after the idempotence load, and no
number grew. The F88 defect shape — an unlatched restart on every load — does
not reproduce, and this time that is measured against a save that genuinely
needed the heal, which the 2026-08-01 sitting could not claim.
⚠️ Sibling result the same cycle, worth knowing when reading that log: leg D1
(cycle 1) found the latch already at `1.0.1` and stable across three loads of an
untouched save — true, but weak, because nothing was going to fire.
...
Same rewrite as F02: the meteor scheduler is a `MapGameTimeRepeat` keyed on `g_NextMeteorsTime` (`Meteors.lua:293-322`), not the global thread this entry was written against.
