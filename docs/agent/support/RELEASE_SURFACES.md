...
The UPLOAD_WORKFLOW §3 backups are the real delivery path, not a fallback: auto-fill from
`metadata.lua` was tested on the v5 upload (2026-08-30) and did not come out clean, so the owner
pasted the backup by hand. Only their styling (headings/bold re-applied in the editor) is
polish — never call the backups themselves optional.
...
Public text is plain enough for players and precise enough for the two Paradox developers who
plan hotfixes from the fix list; it never carries a developer-style hedge ("unverified", "no
guarantees", "not witnessed"). Scope belongs in the bug entry (`agent/bugs/<ID>.md`), not on the
row.
