...
⑤ **public-docs chain** ∥ (`agent/prompts/public-docs/`) — descriptions ×2 **BUILT + swept + arbitrated + TERMINAL AUDIT 2026-08-13: SUSTAINED, with 9 routed corrections** (`reports/STORE_FIXPACK.md` · `STORE_OPTIN.md` · ledger `STORE_BUILD_AUDIT.md`; the audit re-ran evidence+route fresh — its misses cluster in the opt-in module blocks → checklist **22d**, ⛔ NOT applied until the owner's word). 22b CLOSED+STRUCK (bullet re-derived from `F97`); `metadata.lua` strings **APPLIED at owner word** (`1ac1187`); next `05_BUILD_SITE.md`; ③ is now assembly. Feeds ③, does not overtake it. ⭐ **FOUR repos + shared TestKit** — fix pack · opt-in · rescue · **site `C:\Dev\SMR-CommunityMods`** (public, ⛔ nothing on the web: `workflow_dispatch` only, Pages OFF). `MOD_DESCRIPTION.md` FROZEN, **4 known-false claims** — treat every claim in it as unverified.
...
- **Owner decisions open: 5** — **22d** store-copy corrections (rec: approve batch) · 22c hostile-reader page (rec: yes, small) · F85 severity (item 5) · status-vocab word (rec: LEAVE, in 26b) · rescue family-prefix rename (defaulted, release prep). Relabel wording CLOSED via item 22 (08-13). ✅ 08-13: **26 STRUCK — tick
