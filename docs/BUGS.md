...
### F90 — Surface dust storms break UNDERGROUND cables and pipes: the break pass picks its victim from a cross-map grid fragment with no map filter (P2, SOURCE-VERIFIED)  `[built 2026-08-02 by chain prompt 8b as `Code/Fix_DustStormUndergroundBreaks.lua` — the approved §3a LAYER 3 pre-wrapper (surface-only connector list, pcall + restore on both paths, no body copy); probe DustStormBreakMapFilter written. ⚠️ UNRUN — the batch leg is 8b's. The no-yield property was re-traced end to end this session, and one thing the spec had not considered (Break mutates connectors mid-swap) was checked and found symmetric — see the BUILT section]`
...
### F94 — An operator-precedence slip makes ANY parked supply rocket satisfy the asteroid-visit gate (P3, SOURCE-VERIFIED)  `[built 2026-08-02 by chain prompt 8b inside the existing Code/Fix_AsteroidLanderAvailable.lua — §1.4b body copy of PlanetaryView.lua:433-444 (1.0.7.396349) with the brackets restored, F72's scan appended where vanilla's `return false` was; no new registry id. **F72's header AND its BUGS entry were corrected in the same commit** — the chained delegation both advertised is gone. Probe AsteroidVisitPrecedence written. ⚠️ UNRUN — the batch leg is 8b's]`
